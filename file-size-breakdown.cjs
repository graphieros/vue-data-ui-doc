...
function getFilePathParts(filePath) {
    ...
    const extension =
        compoundExtension || path.posix.extname(filenameWithExtension);
...
function stripUidSuffix(filePath) {
    ...
    const originalExtension =
        extension === NO_EXTENSION
            ? ""
            : filenameWithExtension.slice(
                  filenameWithExtension.length - extension.length,
              );
    const cleanFilename = filename.replace(/-[A-Za-z0-9_-]{8,10}$/, "");
    const cleanFilenameWithExtension =
        extension === NO_EXTENSION
            ? cleanFilename
            : `${cleanFilename}${originalExtension}`;
...
function ensureInsideProject(targetPath) {
    ...
    if (relative.startsWith("..") || path.isAbsolute(relative)) {
        throw new Error(
            "Folder must be inside the project that contains this script.",
        );
    }
}
...
function printBreakdown(breakdown) {
    ...
    console.log(
        `Total: ${breakdown.totalSize} across ${breakdown.fileCount} files`,
    );
    console.log("");
...
async function main() {
    ...
    console.log("");
    console.log(
        `Saved JSON result to ${path.relative(process.cwd(), OUTPUT_FILE)}`,
    );
}
