...
describe("landing page", () => {
    ...
    it("should display the app tagline with the proper language", () => {
        ...
        cy.get("@tag").should(
            "contain",
            "Une bibliothèque de composants Vue3 de visualisation de données qui autonomise les utilisateurs",
        );
        cy.get("@langSelect").select("Deutsch");
        cy.get("@tag").should(
            "contain",
            "Eine benutzerorientierte Vue3-Komponentenbibliothek zur Datenvisualisierung",
        );
        cy.get("@langSelect").select("Español");
        cy.get("@tag").should(
            "contain",
            "Una biblioteca de componentes Vue3 de visualización de datos que potencia al usuario",
        );
        cy.get("@langSelect").select("Portugues");
        cy.get("@tag").should(
            "contain",
            "Uma biblioteca de componentes Vue3 para visualização de dados que capacita o usuário",
        );
        cy.get("@langSelect").select("中国人");
        cy.get("@tag").should(
            "contain",
            "一个赋予用户权力的数据可视化Vue3组件库",
        );
        cy.get("@langSelect").select("日本語");
        cy.get("@tag").should(
            "contain",
            "ユーザーをエンパワーするデータ可視化Vue3コンポーネントライブラリ",
        );
        cy.get("@langSelect").select("English");
        cy.get("@tag").should(
            "contain",
            "A user-empowering data visualization Vue3 components library",
        );
    });

    it("should display the header", () => {
        cy.get('[data-cy="app-header"]').should("be.visible");
        cy.get("@linkHome").should("be.visible").contains("Vue Data UI");
        cy.get('[data-cy="header-logo"]').should("be.visible");
        cy.get("@linkInstallation")
            .should("be.visible")
            .contains("Installation");
        cy.get("@linkDocs").should("be.visible").contains("Docs");
        cy.get("@linkDashboard").should("be.visible").contains("Dashboard");
        cy.get("@linkVersions").should("be.visible").contains("Versions");
        cy.get("@linkAbout").should("be.visible").contains("About");
        cy.get("html").should("have.class", "dark");
        cy.get("@btnMode").should("be.visible").click();
        cy.get("html").should("not.have.class", "dark");
        cy.get("@headerSelect").should("be.visible");
    ...
    it("sould call releases api and display the latest version number", () => {
        ...
        cy.intercept(
            "https://vue-data-ui.graphieros.com/releases.json",
            staticResponse,
        );
    });
});
