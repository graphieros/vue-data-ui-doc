...
function updateCrumb() {
    ...
    let hash = route.hash
        ? capitalizeFirstLetter(route.hash.replace("#", "")).replaceAll(
              "-",
              " ",
          )
        : null;
    ...
    const hashIcon = hash
        ? themes.value.find((c) => c.component === hash)
            ? themes.value.find((c) => c.component === hash).icon
            : undefined
        : undefined;
...
const themes = computed(() => {
    return [
        ...
        },
        {
            ...
            backgroundColor: "#f6f6fb",
            color: "#50606C",
            icon: markRaw(DeviceDesktopAnalyticsIcon),
        },
        {
            link: "monochrome",
            name: "Monochrome",
    ...
    <BasePageMenu :items="themes" class="max-w-[1200px] mx-auto mt-6">
        <template #item="{ item }">
            ...
            >
                ...
                <ChevronUpIcon
                    v-if="selectedLink === item.link"
                    class="absolute -bottom-[20px] left-1/2 -translate-x-1/2 text-[#1A1A1A] dark:text-[#CCCCCC]"
                />
            </button>
        </template>
    </BasePageMenu>
