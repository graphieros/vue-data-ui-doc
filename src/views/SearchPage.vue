...
const availableComponents = computed(() => [
    ...new Set(searchResults.value.map((r) => r.componentName)),
]);
...
const filteredResults = computed(() => {
    ...
    return searchResults.value.filter(
        (r) => r.componentName === selectedComponent.value,
    );
});
...
<template>
    ...
    <div
        v-if="!hasResults"
        :class="`mx-auto max-w-[1200px] h-screen ${isOpen ? 'pl-[348px] pr-[48px]' : 'pl-[59px] sm:pl-[109px] pr-12 sm:pr-[59px]'} pt-9 overflow-x-hidden relative z-1`"
    >
        <h1 class="text-2xl border-b border-gray-600 pb-10" dir="auto">
            {{ translations.noResultsFor[store.lang] }}
            <span class="dark:text-app-blue">{{ searchTerm }}</span>
        </h1>
    ...
    <div
        v-else
        :class="`mx-auto max-w-[1200px] min-h-screen ${isOpen ? 'pl-[348px] pr-[48px]' : 'pl-[59px] sm:pl-[109px] pr-12 sm:pr-[59px]'} pt-9 overflow-x-hidden relative z-1`"
    >
        <h1 class="text-2xl mb-6" dir="auto">
            {{ translations.searchResultFor[store.lang] }}
            <span class="dark:text-app-blue">{{ searchTerm }}</span>
            <span class="text-gray-500">({{ filteredResults.length }})</span>
        </h1>
            ...
            >
                        ...
                        <div class="text-[17px]">
                            ...
                            <span
                                v-if="
                                    ...
                                    'Filter by component'
                                "
                                :class="'dark:text-app-blue-light font-inter-medium'"
                                >{{
                                    selectedOption.name.replaceAll("VueUi", "")
                                }}</span
                            >
                            <span v-else :class="'dark:text-app-blue-light'">{{
                                selectedOption.name
                            }}</span>
                        </div>
                    </div>
                ...
                <template #option="{ option, selected, current }">
                    ...
                    >
                        ...
                        <div>
                            <span
                                v-if="option.name !== 'Filter by component'"
                                :class="
                                    selected || current
                                        ? 'text-white'
                            ...
                            <span
                                ...
                                :class="
                                    selected || current
                                        ...
                                        : 'dark:text-app-blue-light font-inter-medium'
                                "
                                >{{
                                    option.translation.replaceAll("VueUi", "")
                                }}</span
                            >
                            <span
                                v-else
        ...
        <div v-for="res in filteredResults" class="my-4 rounded-md">
            <BaseCard class="hover:bg-gray-50 dark:hover:bg-[#3A3A3A]">
                <VueDataUi
                    component="VueUiAccordion"
                    :config="
                        isDarkMode ? accordionConfigDarkMode : accordionConfig
                    "
                >
                    <template
                        #arrow="{ backgroundColor, color, iconColor, isOpen }"
                    >
                        <VueUiIcon
                            name="arrowRight"
                            :size="12"
                    ...
                    <template #title="{ color }">
                        <VueUiIcon
                            :name="iconMap[res.componentName]"
                            stroke="#8A8A8A"
                        />
                        <span
                            class="font-inter-medium text-gray-500 dark:text-[#8A8A8A]"
                            >{{ res.componentName }}:
                        </span>
                    ...
                    <template #content>
                        <div class="flex flex-col w-full">
                            ...
                            <div class="p-4 rounded">
                                <CodeParser
                                    :content="jsonToJsObject(res.value)"
                                    language="javascript"
                                    @copy="store.copy()"
                                >
                                    <template
                                    ...
                                    >
                                        <div
                                            :style="`background:${res.value}`"
                                            class="h-10 w-10 rounded-full border border-gray-400 mr-6"
                                        />
                                    </template>
                                </CodeParser>
                            </div>
                            <router-link :to="`/docs#${res.route}`">
                                <div
                                    class="hover:underline dark:text-app-green font-black mt-2 pl-6"
                                >
                                    {{
                                        translations.search.viewComponent[
                                            store.lang
                                        ]
                                    }}
                                </div>
