...
onMounted(() => {
    ...
    fetch(url.value, {
        ...
        .then((json) => {
            ...
            xyConfig.value.chart.grid.labels.xAxisLabels.values =
                json.downloads.map((d) => d.day);
    ...
    fetch("https://registry.npmjs.org/-/v1/search?text=vue-data-ui", {
        ...
        .then((json) => {
            info.value = json.objects.find(
                (el) => el.package.name === "vue-data-ui",
            ).score.detail;
        });
...
const weekDataset = computed(() => {
    const series = computeSumsByChunks(
        data.value.downloads.map((d) => d.downloads).slice(0, -1),
        7,
    );
...
const sparkbarDataset = computed(() => {
    if (info.value) {
        return Object.keys(info.value)
            .filter((key) =>
                ["maintenance", "popularity", "quality"].includes(key),
            )
            .map((key) => {
                return {
                    name: key,
            ...
            chart: {
                ...
                layout: {
                    wheel: {
                        ticks: {
                            activeColor: "#42d392",
                            inactiveColor: isDarkMode.value
                                ? "#3A3A3A"
                                : "#e1e5e8",
                            gradient: {
...
const wordCloudDataset = computed(() => {
    const source = staticReleases
        .map((r) => {
            ...
            return (
                r.updates
                    .map((u) => singularize(u.description) + " ")
                    .join(" ") + " "
            );
        })
        .join(" ");
...
const KPIS = computed(() => {
    return Object.keys(store.pack)
        .map((key) => {
            if (
                typeof store.pack[key] === "number" &&
                ![
                    "watchers_count",
                    "forks",
                    "watchers",
...
function convertVersionsToTreemap(ds) {
    ...
    return Object.entries(componentCountMap)
...
const carouselDataset = computed(() => {
    ...
    return {
        head: ["Date", "Version", "Component", "Description"],
...
const filteredVersions = computed(() => {
    ...
    const minors = parsed.filter((p) => p.major === maj).map((p) => p.minor);
...
<template>
    ...
    <div
        class="carousel hidden sm:block fixed top-[55px] left-0 w-full"
        style="z-index: 10"
    >
        <VueUiCarouselTable :dataset="carouselDataset" :config="carouselConfig">
            <template #td="{ td, colIndex }">
                ...
                <div
                    ...
                    class="flex flex-row gap-2 place-items-center font-mono text-xs"
                >
                    <VueUiIcon
                        :name="useIconMap(td)"
                        :size="16"
    ...
    <div :class="`pt-9 sm:pt-24 overflow-x-hidden`">
        <div :class="``">
            ...
            <h2
                class="text-[18px] sm:text-[24px] text-center mb-12 text-gray-500"
            >
                {{ dogFood[store.lang] }}
            </h2>
            ...
            <div class="mx-auto max-w-[800px] px-6">
                <BaseCard>
                    <div
                        class="pt-2 flex flex-row flex-wrap align-center gap-4 justify-center place-content-center"
                    >
                        <button
                            v-for="kpi in KPIS"
                            :class="`relative w-full max-w-[150px] button-kpi-${kpi.name.replaceAll(' ', '-')} rounded-md hover:shadow-xl transition-all shadow-[inset_0_2px_2px_#FFFFFF,0_4px_6px_rgba(0,0,0,0.1)] dark:shadow-[inset_0_2px_2px_#4A4A4A,0_4px_6px_rgba(0,0,0,0.5)]`"
                        >
                            <a
                                :href="kpi.link"
                                target="_blank"
                            ...
                            >
                                <VueDataUi
                                    ...
                                    :config="{
                                        ...
                                        valueClass:
                                            'tabular-nums pl-0 sm:pl-0 !drop-shadow-[0_2px_2px_rgba(0,0,0,0.8)]',
                                        analogDigits: {
                                            show: true,
                                            height: 32,
                                ...
                                >
                                    <template #title>
                                        <div
                                            class="absolute top-[21px] right-6 flex-row place-items-center gap-2"
                                            style="transform: scale(1.5, 1.5)"
                                        >
                                            <VueUiIcon
                                                :name="kpi.icon"
                                                :stroke="
                                                    lightenColor(kpi.color, 0.3)
                                                "
                                                :size="20"
                                                :stroke-width="1"
                                            />
                            ...
                            <div
                                class="absolute -top-4 left-0 text-xs text-gray-600 dark:text-[#9A9A9A]"
                            >
                                {{ kpi.displayName }}
                            </div>
                        </button>
            ...
            <div class="mx-auto max-w-[800px] px-6">
                <BaseCard class="max-w-[800px] mx-auto mt-6">
                    <div
                        class="w-full p-4 text-[24px] font-inter-medium flex flex-row gap-2 place-items-center"
                    >
                        <VueUiIcon
                            name="legend"
                        ...
                        <template v-if="enableSem">
                            <label>
                                ...
                                <select
                                    ...
                                    @change="impactMinor"
                                >
                                    <option v-for="o in major.available">
                                        {{ o }}
                                    </option>
                                </select>
                            </label>
                            <label>
                                ...
                                <select
                                    ...
                                    @change="impactPatch"
                                >
                                    <option v-for="o in minor.available">
                                        {{ o }}
                                    </option>
                                </select>
                            </label>
                            <label>
                                ...
                                <select v-model="patch.ver" class="w-[64px]">
                                    <option v-for="o in patch.available">
                                        {{ o }}
                                    </option>
                                </select>
                            </label>
                    ...
                    <div
                        class="flex flex-col sm:flex-row sm:place-items-center gap-2 mb-2 pl-4"
                        v-if="!enableSem"
                    >
                        <label
                            for="versionComponent"
                            @click="openDropdown"
                            class="text-gray-500 dark:text-[#8A8A8A]"
                        ...
                        <div class="flex flex-row gap-2 place-items-center">
                            ...
                            >
                                <template #selected="{ selectedOption }">
                                    <div
                                        v-if="selectedOption"
                                        class="text-left flex flex-row gap-2 place-items-center overflow-x-hidden"
                                    >
                                        <div
                                ...
                                <template
                                    #option="{ option, selected, current }"
                                >
                                    <div
                                        class="text-left flex flex-row gap-2 place-items-center"
                                    >
                    ...
                    <div
                        v-if="!enableSem"
                        class="flex flex-row place-items-center gap-2 mb-6 pl-4 tabular-nums"
                    >
                        <span class="text-gray-500 dark:text-[#8A8A8A]"
                            >Logs:</span
                        >
                    ...
                    <div
                        v-if="
                            !enableSem &&
                            versionComponent === 'vue_ui_vertical_bar'
                        "
                        class="flex flex-row gap-2 place-items-center pl-4 mb-6"
                    >
                        <VueUiIcon
                            name="circleExclamation"
                            :stroke="isDarkMode ? '#ff6600' : '#ff3700'"
                        />
                        <span class="text-app-red dark:text-app-orange">
                            This component was renamed to
                            <code>VueUiHorizontalBar</code> in v3
                        </span>
                    </div>

                    <BaseScroll
                        class="w-full max-h-[500px] p-4 overflow-auto"
                        :fadeColor="isDarkMode ? '#2A2A2A' : '#f3f4f6'"
                    >
                        <ul>
                            <li v-for="log in filteredVersions" :class="`mb-4`">
                                <BaseCard type="light">
                                    <div
                                        class="pt-2 pb-4 mb-6 font-inter-medium text-xl border-b border-gray-300 dark:border-[#5A5A5A] text-[#8A8A8A] mx-6 tabular-nums"
                                    >
                                        {{ log.date }} |
                                        <span
                                            class="text-black dark:text-app-green"
                                            >{{ log.version }}</span
                                        ><br />
                                    </div>
            ...
            <div class="max-w-[800px] mx-auto px-6">
                <BaseCard class="mx-auto mt-6 max-w-[800px]">
                    <div class="max-w-[600px] mx-auto tabular-nums">
                        <VueUiSkeleton
                            v-if="isLoadingLine"
                            :config="sparklineSkeletonConfig"
                        />
                        <VueUiSkeleton
                            v-if="isLoadingLine"
                            :config="sparklineSkeletonConfig"
                ...
                <BaseCard
                    class="max-w-[800px] mx-auto mt-6"
                    v-if="usableHeatmapData.length"
                >
                    <VueUiSkeleton
                        v-if="isLoadingLine"
                        :config="skeletonHeatmapConfig"
                    />
                    <VueUiHeatmap
                        :dataset="usableHeatmapData"
                        :config="heatmapConfig"
                    >
                        <template #source>
                            <div
                                class="text-xs text-gray-500 text-left mt-3 pl-2"
                            >
                                Source:
                                <a class="text-app-blue underline" :href="url"
                                    >api.npmjs.org</a
                ...
                <BaseCard
                    class="mx-auto max-w-[800px] mt-6"
                    v-if="!isLoadingLine && !!data"
                >
                    <VueUiTable
                        :key="`table_${step}`"
                        :dataset="tableDataset"
                        :config="isDarkMode ? tableConfigDarkMode : tableConfig"
                    />
                </BaseCard>
