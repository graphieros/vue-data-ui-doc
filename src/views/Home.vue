...
const deviationY = computed(() => {
    if (clientPosition.value.x > window.innerWidth / 2) {
        ...
        return (
            -(window.innerWidth / 2 - clientPosition.value.x) /
            window.innerWidth
        );
    }
});
...
const deviationX = computed(() => {
    if (clientPosition.value.y > window.innerHeight / 2) {
        ...
        return (
            -(window.innerHeight / 2 - clientPosition.value.y) /
            window.innerHeight
        );
    }
});
    ...
    return [
        {
            ...
            values: [1],
            color: "#647EFF",
            comment: translations.value.menu.installationComment[store.lang],
        },
        {
...
const specialOccasion = computed(() => {
    const starTrigger = [
        1200, 1250, 1300, 1400, 1500, 1600, 1700, 1800, 1900, 2000, 2500, 3000,
        3500, 4000, 4500, 5000, 7500, 10000,
    ].includes(store.stars);
...
<template>
    ...
    <div
        class="w-full fixed left-0 -bottom-2 sm:bottom-5 h-12 sm:h-32 home-sparkline opacity-100"
    >
        <VueUiSparkline
    ...
    <div
        ...
        @mousemove="setClientPosition($event)"
    >
        ...
        >
            <template #td="{ td }">
                <div
                    class="flex flex-row place-items-center gap-4 justify-center"
                >
        ...
        <div
            :class="`absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 custom-styles flex flex-col ${isNarrow ? '' : 'lg:flex-row'} justify-center place-items-center sm:gap-[80px] w-full max-w-[1450px]`"
        >
            <div
                class="flex flex-col gap-6 max-w-[360px] sm:max-w-[450px] justify-center place-items-center text-center"
            >
                <h1 style="font-family: InterBold" class="text-[49px]">
                    <!-- <span class="">Vue </span> -->
                    <span
                        class="bg-gradient-to-r from-app-green to-indigo-400 bg-clip-text text-transparent"
                        style="letter-spacing: -1px"
                        >Vue Data UI</span
                    >
                </h1>
                <p
                    class="font-inter-medium text-board-2 text-xl sm:text-2xl text-gray-700 dark:text-gray-400"
                >
                    {{ translations.tagline[store.lang] }}
                </p>

                <div
                    class="flex flex-row gap-2 text-gray-500 relative"
                    :title="componentsLen + ' components'"
                >
                    {{ componentTranslation[store.lang] }}:
                    <div class="h-[22px]">
                        <VueUiDigits
                            :dataset="componentsLen"
                            :config="{
                                ...digitConfigStars,
                                digits: {
                                    ...digitConfigStars.digits,
                                    color: isDarkMode ? '#647EFF' : '#6A6A6A',
                                    thickness: 2,
                                },
                            }"
                        />
                ...
                >
                    <div
                        class="p-0.5 bg-gradient-to-r from-app-green to-app-blue rounded-full"
                    >
                        <button
                            class="relative flex flex-row place-content-center place-items-center bg-white dark:bg-black from-app-green to-app-blue py-3 px-5 rounded-full text-black dark:text-gray-400 font-inter-medium hover:shadow-xl dark:hover:bg-[#2A2A2A] gap-3 transition-all"
                        >
                            ...
                            <div
                                ...
                                v-if="store.stars && !store.isFetching"
                            >
                                <div class="relative">
                                    <StarFilledIcon
                                        class="text-[#fdd663] drop-shadow-sm"
                                    />
                                    <!-- FOR SPECIAL OCCASIONS -->
                                    <svg
                                        viewBox="0 0 20 20"
                                        height="16"
                                        class="absolute top-[6px] left-1"
                                        v-if="specialOccasion"
                                    >
                                        <path
                                            d="M 7 7 L 7 4 M 13 7 L 13 4 Z M 5 9 C 7 14 13 14 15 9"
                                            fill="none"
                                            stroke="black"
                                            stroke-width="1.3"
                                ...
                                <span
                                    class="text-xs dark:text-[#fdd663] h-[20px]"
                                >
                                    <Suspense>
                                        <template #default>
                                            <VueUiDigits
                                                :dataset="store.stars"
                                                :config="digitConfigStars"
                                            />
                                        </template>
                                        <template #fallback>
                ...
                <!-- When we are ready -->
                <div
                    v-if="versionsList.length"
                    class="flex flex-row place-items-center h-[30px] mx-auto w-full justify-center mt-0.5"
                >
                    <Suspense>
                        <template #default>
                            <a
                ...
                <div
                    v-else
                    class="flex flex-row place-items-center h-[30px] mx-auto w-full justify-center"
                ></div>
            </div>

            <div
                class="w-[400px] max-w-[400px] lg:w-[500px] lg:min-w-[500px] 2xl:w-[900px] 2xl:min-w-[900px] relative transition-all"
            >
                <Suspense>
                    <template #default>
                        <div>
                            <VueUiDonut
                                :dataset="datasetDonutMenu"
                                :config="configDonutMenu"
                                @selectDatapoint="selectMenu"
                            >
                    ...
                    `"
                                    >
                                        <div
                                            :class="`text-[#3A3A3A] dark:text-[#CCCCCC] font-inter-medium text-[12px] sm:text-[16px] ${plot.patternIndex === 4 ? 'pb-2' : ''}`"
                                            style="letter-spacing: -1px"
                                        >
                                            {{ plot.name }}
                                        </div>
                            ...
                            <div
                                class="home-perspective-wrapper flex flex-col gap-6 sm:gap-12 max-w-[500px] place-items-center absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-[20px] pointer-events-none"
                            >
                                <div
                                    class="relative z-10 home-perspective"
                                    :style="`transform: rotateY(${deviationY * 30}deg) rotateX(${-deviationX * 20}deg);`"
                                >
                                    <img
                                        data-cy="app-logo"
        ...
        <button
            @click="changeTheme"
            id="themeToggle"
            class="flex place-items-center place-content-end w-full py-1 pr-4 text-center absolute top-3 right-2"
        >
        ...
        <svg
            :viewBox="viewBox"
            width="100%"
            class="bg-transparent absolute top-0 left-0 user-position pointer-events-none"
        >
            <line
                :x1="clientPosition.x"
                :x2="clientPosition.x"
        ...
        >
            ...
            >
                <div
                    style="transform: rotate(-90deg); height: 16px; width: 46px"
                >
                    <VueUiDigits
...
<style>
.custom-styles .vue-ui-donut svg {
