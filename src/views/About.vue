    ...
    return [
        {
            ...
            ja: "Vueネイティブ、ランタイム依存なし – npmでは依存が0と表示されており、ライブラリコードのみを配布できます",
        ...
        },
        {
            ...
            ja: `既製コンポーネント ${componentsLen.value} 個`,
        ...
        },
        {
            ...
            ja: "「ユニバーサル」コンポーネントにより、ほとんどの画面で1つのコンポーネントだけを遅延読み込みし、実行時にチャートタイプを切り替えられます",
        ...
        },
        {
            ...
            ja: "「ユーザーオプション」バーガーメニューを標準搭載 – PDF/PNG/CSV出力、全画面表示、ツールチップ切替、テーブル表示、注釈オーバーレイなど、すべてのチャートタイプで対応",
        ...
        },
        {
            ...
            ja: "各コンポーネントにプログラマブルなヘルパー機能あり（generatePdf()、toggleTable()、getData() など）",
        ...
        },
        {
            ...
            ja: "ダッシュボードとユーティリティウィジェット（VueUiDashboard、VueUiKpi、アコーディオン、タイマー、ローダーなど）を使って、複数のUIキットを混在させずに完全なBIパネルを構築可能",
...
const target = computed(() => {
    const activeThreshold = store.thresholds.find(
        (t) =>
            currentStars.value >= (t?.min ?? 0) &&
            currentStars.value < (t?.max || 1),
    );
...
<template>
    ...
    <div
        class="flex flex-col place-content-center place-items-center text-left mt-12 w-5/6 sm:w-1/2 mx-auto"
    >
        <div
            class="w-full flex flex-row gap-4 place-items-center justify-center mt-12"
        >
            <VueUiIcon
      ...
      <div
        ...
        <BaseCard class="w-full">
            ...
            <BaseCard type="light">
                <div class="w-full mb-3 overflow-x-auto">
                    <table
                        class="w-full border-separate border-spacing-y-0 text-sm text-gray-800 dark:text-gray-200"
                    >
                        <tbody>
                            <tr
                            ...
                            >
                                <td
                                    class="py-1 rounded-md flex flex-row place-items-center gap-4"
                                >
                                    <div class="w-[40px]">
            ...
            <div class="px-2">
                ...
                <p class="my-3" dir="auto">
                    ...
                    <span
                        class="text-black font-bold dark:text-app-green hover:underline"
                        ><router-link to="/docs#vue-ui-chestnut"
                            >VueUiChestnut</router-link
                        ></span
                    >,
                    <span
                        class="text-black font-bold dark:text-app-green hover:underline"
                        ><router-link to="/docs#vue-ui-quadrant"
                            >VueUiQuadrant</router-link
                        ></span
                    >,
                    <span
                        class="text-black font-bold dark:text-app-green hover:underline"
                        ><router-link to="/docs#vue-ui-table"
                            >VueUiTable</router-link
                        ></span
                    >.
                </p>
            </div>
            ...
            <BaseCard type="light" class="mt-10" v-if="contributors.length">
                <div
                    class="w-full text-app-blue dark:text-app-blue-light font-inter-medium text-2xl mb-6 text-center"
                >
                    {{ translations.about.contributors[store.lang] }}
                </div>
                <div
                    class="flex flex-row flex-wrap gap-2 justify-center mx-auto max-w-[300px]"
                >
                    <div v-for="contributor in contributors">
                        <img
                            :src="contributor.avatar_url"
                            :title="contributor.login"
                            class="rounded-full w-10 h-10 border border-[#CCCCCC]"
                        />
                    </div>
                </div>
                <ContributorsBars />
            ...
            <BaseCard type="light" class="mt-10">
                <div
                    class="w-full text-app-blue dark:text-app-blue-light font-inter-medium text-2xl"
                >
                    {{ translations.about.maintainers[store.lang] }}
                </div>
                <div
                    class="w-full mt-4 flex place-items-center place-content-center"
                >
                    <div
                        class="flex gap-6 flex-col md:flex-row place-items-center place-content-center"
                    >
                        <img
                            src="../assets/alec_lloyd_probert.png"
                            class="h-[200px] rounded"
                            alt="A picture of Alec Lloyd Probert"
                        />
                        <p dir="auto">
                            {{ translations.about.alp[store.lang] }}<br /><br />
                            <a
                                class="text-app-blue dark:text-app-blue-light hover:underline"
                                href="https://github.com/graphieros"
                                target="_blank"
                            >
                                <button
                                    class="flex flx-row place-items-center gap-2"
                                >
        ...
        <div
            v-if="isDarkMode"
            class="w-full grid grid-cols-3 gap-6 mt-12 mx-auto max-w-[500px] mb-12"
        >
            <div
                v-for="skeleton in skeletonsDarkMode.slice(3, skeletons.length)"
                class="max-w-1/3"
            >
        ...
        >
            <div
                v-for="skeleton in skeletons.slice(3, skeletons.length)"
                class="max-w-1/3"
            >
                <VueUiSkeleton :config="skeleton" />
            </div>
