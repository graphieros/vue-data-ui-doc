...
function isSelected(item, index) {
    return (
        ...
        (index === 0 && currentRoute.value === "/customization")
    );
}
    ...
    (v) => {
        ...
        if (!hash) {
            ...
            const currentMenu = menu.value.find(
                (el) => el.link.split("#")[1] === hash.replaceAll("#", ""),
            );
...
<template>
        ...
        <BasePageMenu :items="menu">
            <template #item="{ item, index }">
                ...
                >
                    ...
                    >
                        <div
                            class="h-fit w-fit flex place-items-center justify-center relative"
                        >
                            <ColorBridgeIcon
                                v-if="item.name === 'colorBridge'"
                            />
                            <component
                                v-else
                                :is="item.icon"
    ...
    <h2
        class="text-center text-4xl font-inter-medium w-full mx-auto max-w-[1200px]"
    >
    ...
    <QuickCustomTheme
        v-if="
            currentRoute === '/customization#quick-custom-theme' ||
