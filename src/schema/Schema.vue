    ...
    <div class="schema p-4 w-full max-w-[600px]">
        <div class="mb-4 flex flex-row gap-4">
            <VueUiIcon
                ...
                :stroke="isDarkMode ? '#559AD3' : '#6A6A6A'"
            />
            <div class="text-xl font-inter-medium dark:text-[#9cdcfe]">
                {{ selectedComponent.name }}
            </div>
        </div>
        <div class="mb-4">
            <BaseText
                :label="translations.overview.description[store.lang]"
                :text="
                    translations.docs.tooltips[selectedComponent.description][
                        store.lang
                    ]
                "
            />
            <div class="flex flew-row gap-2 place-items-center">
                ...
                >
                    <code>
                        <span class="dark:text-[#4dc6ae]">{{
                            selectedComponent.types.dataset.includes("[]")
                                ? selectedComponent.types.dataset.replaceAll(
                                      "[]",
                                      "",
                                  )
                                : selectedComponent.types.dataset
                        }}</span>
                        <span
                            class="text-[#8A8A8A]"
                            v-if="
                ...
                <button
                    title="Copy"
                    class="dark:text-[#9A9A9A]"
                    @click="copyToClipboard(selectedComponent.types.dataset)"
                >
                    <CopyIcon />
                </button>
            </div>
            <div class="flex flew-row gap-2 place-items-center">
                ...
                <button
                    title="Copy"
                    class="dark:text-[#9A9A9A]"
                    @click="copyToClipboard(selectedComponent.types.config)"
                >
                    <CopyIcon />
                </button>
            </div>
            ...
            <div class="mt-8 mb-2" dir="auto">
                {{ translations.universalBoilerplate[store.lang] }}
            </div>
            <div class="flex flex-row gap-2 place-items-center mb-2" dir="auto">
                <label for="useTS">{{ translations.useTs[store.lang] }}</label
                ><input type="checkbox" v-model="isTs" id="useTS" />
            </div>
            <div class="bg-black text-gray-400 p-2 sm:p-6 rounded relative">
                <button
                    title="Copy"
                    class="text-[#6A6A6A] absolute right-4 top-4"
                    @click="copyContent"
                >
                    <CopyIcon />
                </button>
...
&lt;<span class="code-html">template</span>&gt;
    &lt;<span class="code-html">{{ selectedComponent.name }}</span>
...
&lt;/<span class="code-html">template</span>&gt;
        ...
        <table
            class="schema table-auto border-collapse border border-slate-500 my-4 w-full"
        >
            <caption
                class="caption-top py-4 text-left text-xl font-inter-medium"
            >
            ...
            <thead>
                <tr class="bg-[#FFFFFF80] dark:bg-[#FFFFFF05]">
                    <th class="border border-slate-600 p-2">
                        {{ translations.overview.themes[store.lang] }}
                    </th>
                    ...
                    <th class="border border-slate-600 p-2">
                        Responsive<sup v-if="!hideDisclaimer">1</sup>
                    </th>
                </tr>
            </thead>
            <tbody>
        ...
        <span v-if="!hideDisclaimer" dir="auto">
            1. {{ translations.responsiveUsage[store.lang] }}
            {{ translations.responsive[store.lang] }}
        </span>

        <div
            v-if="selectedComponent.hasLttb"
            class="mt-6 bg-[#42d39230] dark:bg-[#42d39210] py-2 px-4 border-l-2 border-app-green font-black"
            dir="auto"
        >
        ...
        <table
            v-if="selectedComponent.userOptions"
        ...
        >
            ...
            <thead>
                <tr class="bg-[#FFFFFF80] dark:bg-[#FFFFFF05]">
                    ...
                    <th class="border border-slate-600 p-2">
                        {{ translations.overview.description[store.lang] }}
                    </th>
                </tr>
            </thead>
            <tbody>
                <tr
                    ...
                    v-for="option in selectedComponent.userOptions"
                >
                    <td class="p-2 border border-slate-600">
                        <div class="w-full flex justify-center">
                            <VueUiIcon
        ...
        >
            ...
            <tbody>
                <tr
                    class="bg-[#FFFFFF60] dark:bg-[#FFFFFF10]"
                    v-for="option in selectedComponent.slots"
                >
                    <td
                        class="p-2 border border-slate-600 text-xs dark:bg-[#1A1A1A]"
                    >
        ...
        >
            ...
            <tbody>
                <tr
                    class="bg-[#FFFFFF60] dark:bg-[#FFFFFF10]"
                    v-for="option in selectedComponent.emits"
                >
                    <td
                        class="p-2 border border-slate-600 text-xs dark:bg-[#1A1A1A]"
                    >
        ...
        <table
            v-if="selectedComponent.exposed"
            class="table-auto border-collapse border border-slate-500 my-4 w-full"
        >
            <caption
                class="caption-top py-4 text-left text-xl font-inter-medium"
            ...
            <tbody>
                <tr
                    class="bg-[#FFFFFF60] dark:bg-[#FFFFFF10]"
                    v-for="option in selectedComponent.exposed"
                >
                    <td
                        class="p-2 border border-slate-600 text-xs dark:bg-[#1A1A1A]"
                    >
        ...
        <div
            :class="`w-full mx-auto p-4 bg-white ${selectedComponent.chartSize ? `max-w-[${selectedComponent.chartSize}px]` : ''}`"
