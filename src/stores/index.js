        ...
        return {
            ...
            translations: {
                ...
                annotationsYTop: {
                    ...
                    zh: '提供一个符合您数据集范围的数字',
                    jp: 'データセットのスケールに合わせた数値を提供してください',
                ...
                annotationsYBottom: {
                    ...
                    jp: 'データセットのスケールに合わせた別の数値を提供してください。yTopと異なる場合、エリアが表示されます',
                ...
                getTheme: {
                    en: "You can apply one of the 5 themes to all charts. Just set the 'theme' config attribute to either '', 'zen', 'hack', 'concrete', 'celebration', or 'celebrationNight'. Theme objects can be retrieved by importing and calling the getThemeConfig function, passing as a parameter the key corresponding to the component.",
                    fr: "Vous pouvez appliquer l'un des 5 thèmes à tous les graphiques. Il suffit de définir l'attribut de configuration 'theme' sur '', 'zen', 'hack', 'concrete', 'celebration' ou 'celebrationNight'. Les objets de thème peuvent être récupérés en important et en appelant la fonction getThemeConfig, en passant comme paramètre la clé correspondant au composant.",
                    pt: "Você pode aplicar um dos 5 temas a todos os gráficos. Basta definir o atributo de configuração 'theme' como '', 'zen', 'hack', 'concrete', 'celebration' ou 'celebrationNight'. Os objetos de tema podem ser recuperados importando e chamando a função getThemeConfig, passando como parâmetro a chave correspondente ao componente.",
                    de: "Sie können eines der 5 Themes auf alle Diagramme anwenden. Setzen Sie einfach das Konfigurationsattribut 'theme' auf '', 'zen', 'hack', 'concrete', 'celebration' oder 'celebrationNight'. Theme-Objekte können abgerufen werden, indem Sie die Funktion getThemeConfig importieren und aufrufen und den Schlüssel des entsprechenden Komponenten übergeben.",
                    zh: "您可以将 5 个主题之一应用于所有图表。只需将 'theme' 配置属性设置为 '', 'zen', 'hack', 'concrete', 'celebration' 或 'celebrationNight'。通过导入并调用 getThemeConfig 函数，传入与组件对应的键，即可检索主题对象。",
                    jp: "5つのテーマのいずれかをすべてのチャートに適用できます。'theme'設定属性を'', 'zen', 'hack', 'concrete', 'celebration', または 'celebrationNight'に設定するだけです。テーマオブジェクトは、getThemeConfig関数をインポートして呼び出し、コンポーネントに対応するキーをパラメーターとして渡すことで取得できます。",
                    es: "Puedes aplicar uno de los 5 temas a todos los gráficos. Simplemente configura el atributo 'theme' como '', 'zen', 'hack', 'concrete', 'celebration' o 'celebrationNight'. Los objetos de tema se pueden obtener importando y llamando a la función getThemeConfig, pasando como parámetro la clave correspondiente al componente.",
                    ko: "5가지 테마 중 하나를 모든 차트에 적용할 수 있습니다. 'theme' 설정 속성을 '', 'zen', 'hack', 'concrete', 'celebration' 또는 'celebrationNight'으로 설정하기만 하면 됩니다. 테마 객체는 getThemeConfig 함수를 가져와 호출하고 해당 컴포넌트의 키를 매개변수로 전달하여 가져올 수 있습니다.",
                    ar: "يمكنك تطبيق أحد الموضوعات الخمسة على جميع الرسوم البيانية. فقط قم بتعيين سمة التكوين 'theme' إلى '', 'zen', 'hack', 'concrete', 'celebration' أو 'celebrationNight'. يمكن استرداد كائنات السمات عن طريق استيراد واستدعاء وظيفة getThemeConfig، وتمرير المفتاح المقابل للمكون كمعامل."
                },
                getConfig: {
                ...
                slots: {
                    ...
                    tooltip: {
                        ...
                        ar: `يحتوي هذا المكون على "slots" #tooltip-before و#tooltip-after لتخصيص محتويات التلميح. من خلال هذه الـ "slots"، يمكنك إدخال صور، مخططات، نصوص، إلخ.`
                    },
                    legendDetail: {
                ...
                menu: {
                    docsComment: {
                        ...
                        ar: 'توثيق مفصل وتفاعلي وشامل مع أمثلة.'
                    },
                    builderComment: {
                        ...
                        ar: 'قم بسرعة بإنشاء مخطط ونسخ كود المكون'
                    },
                ...
                docs: {
                    ...
                    tooltips: {
                        ...
                        chord: {
                            ...
                            fr: "Révéler les schémas de flux ou de connectivité dans des données matricielles ou en réseau complexes à l'aide d'un graphique circulaire utilisant des arcs pour chaque catégorie et des rubans pour coder la force et la direction des relations entre elles.",
                            pt: "Revelar padrões de fluxo ou conectividade em dados matriciais ou de rede complexos com um gráfico circular que usa arcos para cada categoria e fitas para codificar a força e a direção dos relacionamentos entre elas.",
                            de: "Zeigt Fluss- oder Konnektivitätsmuster in komplexen Matrix- oder Netzwerkdaten mithilfe eines Kreisdiagramms, das Bögen für jede Kategorie und Bänder verwendet, um die Stärke und Richtung der Beziehungen zwischen ihnen zu kodieren.",
                            zh: "通过使用圆形图表来揭示复杂矩阵或网络数据中的流动或连接模式，该图表为每个类别使用弧，并使用带状图编码它们之间关系的强度和方向。",
                            jp: "複雑な行列やネットワークデータにおけるフローや接続性のパターンを、各カテゴリのアークと関係の強さと方向を表すリボンを用いた円形チャートで明らかにします。",
