                ...
                docs: {
                    ...
                    comments: {
                        ...
                        xy: {
                            ...
                            continuousX: {
                                en: "Use `number[]` for category-based charts where x values are implicit and mapped to labels, dates, or ordered positions on the x-axis. Use `{ x, y }[]` for continuous datasets where both coordinates are explicitly defined and x values may be irregularly spaced. Continuous (x,y) series are supported since v3.20.0 and are only available for 'line' and 'plot' chart types.",
                                fr: "Utilisez `number[]` pour les graphiques basés sur des catégories où les valeurs x sont implicites et associées à des labels, dates ou positions ordonnées sur l’axe x. Utilisez `{ x, y }[]` pour les séries continues où les deux coordonnées sont explicitement définies et où les valeurs x peuvent être espacées de manière irrégulière. Les séries continues (x,y) sont prises en charge depuis la v3.20.0 et uniquement disponibles pour les types 'line' et 'plot'.",
                                pt: "Use `number[]` para gráficos baseados em categorias, onde os valores de x são implícitos e associados a rótulos, datas ou posições ordenadas no eixo x. Use `{ x, y }[]` para séries contínuas em que ambas as coordenadas são explicitamente definidas e os valores de x podem ter espaçamentos irregulares. Séries contínuas (x,y) são suportadas desde a v3.20.0 e estão disponíveis apenas para os tipos 'line' e 'plot'.",
                                de: "Verwenden Sie `number[]` für kategoriebasierte Diagramme, bei denen x-Werte implizit sind und Labels, Daten oder geordneten Positionen auf der x-Achse zugeordnet werden. Verwenden Sie `{ x, y }[]` für kontinuierliche Datensätze, bei denen beide Koordinaten explizit definiert sind und x-Werte unregelmäßig verteilt sein können. Kontinuierliche (x,y)-Reihen werden seit v3.20.0 unterstützt und sind nur für die Typen 'line' und 'plot' verfügbar.",
                                zh: "对于基于类别的图表，请使用 `number[]`，其中 x 值是隐式的，并映射到标签、日期或 x 轴上的顺序位置。对于连续数据集，请使用 `{ x, y }[]`，其中两个坐标都是显式定义的，并且 x 值可以是不规则间隔的。连续 (x,y) 序列自 v3.20.0 起受支持，并且仅适用于 'line' 和 'plot' 类型。",
                        ...
                        waffle: {
                            gridSize: {
                                en: "results in a 10 x 10 grid",
                                fr: "grille 10 x 10",
                                pt: "resulta em uma grade de 10 x 10",
                                de: "ergibt ein 10 x 10 Raster",
                                zh: "结果是 10 x 10 网格",
                                ja: "結果は 10 x 10 グリッドになります",
                                es: "da como resultado una cuadrícula de 10 x 10",
                                ko: "결과는 10 x 10 그리드입니다.",
                                ar: "النتائج في شبكة 10 × 10",
                            },
                        },
                        radar: {
                            seriesValues: {
