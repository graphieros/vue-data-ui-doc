...
<template>
    <Transition name="fade">
        <div
            v-if="!alreadyRated && visible && store.ratings.show"
            class="relative w-fit flex flex-col justify-center place-items-center gap-3 mx-auto my-4 bg-gray-100 dark:bg-[#242424] p-4 rounded-xl shadow-md shadow[inset_0_2px_2px_#FFFFFF,0_4px_6px_rgba(0,0,0,0.1)] dark:shadow-[inset_0_2px_2px_#4A4A4A,0_4px_6px_rgba(0,0,0,0.5)]"
        >
            <VueUiRating
                :dataset="{ rating }"
                :config="config"
            ...
            />
            <button
                class="whitespace-nowrap py-1 px-4 outline rounded-full bg-gradient-to-br disabled:from-[#8A8A8A] disabled:to-[#9A9A9A] dark:disabled:from-[#242424] dark:disabled:to-[#242424] dark:disabled:outline-[#3A3A3A] dark:disabled:text-[#5A5A5A] from-[#f7e6b0] to-[#f5bb0a] disabled:text-[#CCCCCC] disabled:cursor-not-allowed text-black"
                :disabled="!rated || done"
                @click="submitRating"
            >
