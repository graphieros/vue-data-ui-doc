<template>
    ...
    >
        ...
        <template #watermark v-if="loading">
            <div>
                <BaseBubbles class="mt-5" randomColor stroke="#FFFFFF40" />
            </div>
        </template>
    </VueDataUi>
</template>
...
const loadingDataset = computed(() => {
    return [
        {
            name: loadingLabel.value[store.lang],
            color: isDarkMode.value ? "#6A6A6A" : "#8A8A8A",
            values: [1],
        },
        {
            name: loadingLabel.value[store.lang],
            color: isDarkMode.value ? "#8A8A8A" : "#9F9F9F",
            values: [1],
        },
        {
            name: loadingLabel.value[store.lang],
            color: isDarkMode.value ? "#AFAFAF" : "#BFBFBF",
            values: [1],
        },
    ];
});
