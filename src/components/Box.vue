...
const menuItems = computed(() => {
    return [
        ...
        {
            name: menuTranslations.value.tipsAndTricks[store.lang],
            icon: "lightBulbOn",
            color: isDarkMode.value ? "#fdd663" : "#FFFFFF",
...
<template>
    <Transition name="fade">
        ...
        >
            ...
            >
                <button
                    @click="() => selectTabFromMini(tab.order)"
                    :class="`p-2 hover:bg-[#FAFAFA] dark:hover:bg-[#3A3A3A] bg-gray-100 dark:bg-[#2A2A2A] rounded-full hover:bg-gradient-to-tl transition-colors relative shadow-[inset_0_2px_2px_#FFFFFF,0_4px_6px_rgba(0,0,0,0.1)] dark:shadow-[inset_0_2px_2px_#4A4A4A,0_4px_6px_rgba(0,0,0,0.5)]`"
                    :style="{
                        outline:
                            activeTab === tab.order
                                ? `1px solid ${tab.color}90`
                                : undefined,
                        background:
                            activeTab === tab.order && !isDarkMode
                                ? 'radial-gradient(at top, white, #abc2f6)'
                                : undefined,
                    }"
                >
                    <IconSettings
                    ...
                    <VueUiIcon
                        v-else
                        :name="tab.icon"
                        :stroke="isDarkMode ? tab.color : '#2A2A2A'"
                        :size="20"
                    />
                    <div
                        v-if="activeTab === tab.order"
    ...
    <div
        v-if="schema"
        class="rounded-xl pl-4 my-6 relative overflow-x-auto dark:bg-[#242424] bg-gradient-to-br from-transparent to-[#5F8BEE20] shadow-[inset_0_2px_2px_#FFFFFF,0_4px_6px_rgba(0,0,0,0.1)] dark:shadow-[inset_0_2px_2px_#4A4A4A,0_4px_6px_rgba(0,0,0,0.5)]"
    >
        <VueDataUi
            ...
            <template #content>
                <div class="mx-auto w-fit">
                    <Schema
                        :component="schema"
                        :showLink="false"
                        :hideDisclaimer="hideSchemaDisclaimer"
                    />
                </div>
            </template>
        </VueDataUi>
    ...
    <div
        ref="boxWrapper"
        class="p-6 rounded-xl my-6 relative overflow-x-auto bg-gray-100 dark:bg-[#242424] scroll-mt-8 shadow-[inset_0_2px_2px_#FFFFFF,0_4px_6px_rgba(0,0,0,0.1)] dark:shadow-[inset_0_2px_2px_#4A4A4A,0_4px_6px_rgba(0,0,0,0.5)]"
    >
        ...
        <div ref="tabsMenu">
            <BaseTabsMenu
                :items="menuItems"
                @tabSelected="({ order }) => (activeTab = order)"
                :activeTab="activeTab"
            />
        </div>
        ...
        <div
            class="text-app-orange text-lg my-3 gap-2 flex flex-row align-center"
        >
        ...
        <div v-if="activeTab === 8">
            ...
            <div
                v-if="patternTranslations && patternTranslations.moreInfo"
                class="flex place-items-center justify-center my-6"
            >
                <RouterLink :to="{ path: '/customization', hash: '#patterns' }">
                    <button
                        class="relative py-2 px-6 rounded-full bg-[#FF7F7F] text-black opacity-90 hover:opacity-100 hover:shadow-md transition-all"
                    >
