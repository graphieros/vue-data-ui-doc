...
<template>
    ...
    <div class="p-6 rounded-md border border-gray-700 my-6 relative overflow-x-auto">
        ...
        <div tabindex="0" :class="`w-fit select-none cursor-pointer text-black dark:text-app-blue font-satoshi-bold p-3 border-b   ${activeTab === 1 ? 'bg-gray-200 dark:bg-black-100 border-b-app-blue' : 'border-b-transparent'} hover:border-b hover:border-b-app-blue`" @click="activeTab = 1; emit('onNonResponsiveTab')" @keypress.enter="activeTab = 1; emit('onNonResponsiveTab')">
            <div class="flex flex-row place-items-center gap-2 w-fit whitespace-nowrap">
                <IconSettings :stroke="isDarkMode ? '#5f8bee' : '#1A1A1A'" :size="26"/>
                Config
            </div>
        </div>
        <div tabindex="0" v-if="props.showEmits" :class="`w-fit select-none cursor-pointer text-black dark:text-gray-400  border-b   font-satoshi-bold p-3 ${activeTab === 2 ? 'bg-gray-200 dark:bg-black-100 border-b-gray-100' : 'border-b-transparent'} hover:border-b hover:border-b-gray-100`" @click="activeTab = 2; emit('onNonResponsiveTab')" @keypress.enter="activeTab = 2; emit('onNonResponsiveTab')">
            <div class="flex flex-row place-items-center gap-2 w-fit whitespace-nowrap">
                <VueUiIcon name="func" :size="18" :stroke="isDarkMode ? '#CCCCCC' : '#1A1A1A'"/>
                Emits
            </div>
        </div>
        <div tabindex="0" v-if="props.showSlots" :class="`w-fit select-none cursor-pointer text-black dark:text-app-orange border-b   font-satoshi-bold p-3  ${activeTab === 3 ? 'bg-gray-200 dark:bg-black-100 border-b-app-orange' : 'border-b-transparent'} hover:border-b hover:border-b-app-orange`" @click="activeTab = 3; emit('onNonResponsiveTab')" @keypress.enter="activeTab = 3; emit('onNonResponsiveTab')">
            <div class="flex flex-row place-items-center gap-2 w-fit whitespace-nowrap">
                <BracketsContainIcon size="18"/>
                Slots
            </div>
        </div>
        <div tabindex="0" v-if="props.showTooltip" :class="`w-fit select-none cursor-pointer text-black dark:text-gray-100 border-b   font-satoshi-bold p-3  ${activeTab === 4 ? 'bg-gray-200 dark:bg-black-100 border-b-gray-100' : 'border-b-transparent'} hover:border-b hover:border-b-gray-100`" @click="activeTab = 4; emit('onNonResponsiveTab')" @keypress.enter="activeTab = 4; emit('onNonResponsiveTab')">
            <div class="flex flex-row place-items-center gap-2 w-fit whitespace-nowrap">
                <VueUiIcon name="tooltip" :size="18" :stroke="isDarkMode ? '#CCCCCC' : '#1A1A1A'"/>
                Custom tooltip
            </div>
        </div>
        <div tabindex="0" v-if="props.showUseCases" :class="`w-fit select-none cursor-pointer text-black dark:text-app-gold border-b font-satoshi-bold p-3  ${activeTab === 5 ? 'bg-gray-200 dark:bg-black-100 border-b-app-gold' : 'border-b-transparent'} hover:border-b hover:border-b-app-gold`" @click="activeTab = 5; emit('onNonResponsiveTab')" @keypress.enter="activeTab = 5; emit('onNonResponsiveTab')">
            <div class="flex flex-row place-items-center gap-2 w-fit whitespace-nowrap">
                <VueUiIcon name="clipboardLine" :size="18" :stroke="isDarkMode ? '#fdd663' : '#1A1A1A'"/>
                Use cases
            </div>
        </div>
        <div tabindex="0" v-if="props.showThemes" :class="`w-fit select-none cursor-pointer text-black dark:text-[#ddaaFF] border-b font-satoshi-bold p-3  ${activeTab === 6 ? 'bg-gray-200 dark:bg-black-100 border-b-[#ddaaFF]' : 'border-b-transparent'} hover:border-b hover:border-b-[#ddaaFF]`" @click="activeTab = 6; emit('onNonResponsiveTab')" @keypress.enter="activeTab = 6; emit('onNonResponsiveTab')">
            <div class="flex flex-row place-items-center gap-2 w-fit whitespace-nowrap">
                <VueUiIcon name="clipboardBar" :size="18" :stroke="isDarkMode ? '#ddaaFF' : '#1A1A1A'"/>
                Themes
            </div>
        </div>
        <div tabindex="0" v-if="props.showResponsive" :class="`w-fit select-none cursor-pointer text-black dark:text-[#40b3c7] border-b font-satoshi-bold p-3  ${activeTab === 7 ? 'bg-gray-200 dark:bg-black-100 border-b-[#40b3c7]' : 'border-b-transparent'} hover:border-b hover:border-b-[#40b3c7]`" @click="setResponsiveTab()" @keypress.enter="setResponsiveTab()">
            <div class="flex flex-row place-items-center gap-2 w-fit whitespace-nowrap">
