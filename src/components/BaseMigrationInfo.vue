...
const translations = computed(() => {
    return {
        ...
        padding: {
            ...
            ja: "パディングの設定が変更され、旧設定のままだと過剰なパディングが発生する可能性があります。デフォルトのパディング設定を使用しても問題ありません。不要になっている可能性のある他のラベルのオフセットも確認してください。",
        ...
        debug: {
            ...
            ja: "開発中は config.debug を有効にすると、データセットが不完全または無効な場合に情報的な警告が表示されます。",
        ...
        cssAnimation: {
            ...
            ja: "config.useCssAnimation はデフォルトで false に設定されるようになりました",
        ...
        zoomPreview: {
            ...
            ja: "新しいズームプレビュー機能はデフォルトで有効になっています。以前の挙動に戻すには、config.chart.zoom.preview.enable を false に設定してください。",
        ...
        autoRotate: {
            ...
            ja: "一部のラベルは衝突時に自動回転するようになりました。ラベル設定で autoRotate.enable を false にすると、以前の挙動に戻せます。自動回転の角度もそこで設定できます。",
        ...
        xyTransitions: {
            ...
            ja: "シリーズタイプごとに CSS トランジションを無効化します。config.line.showTransition、config.bar.showTransition、config.plot.showTransition で false に設定すると、CSS トランジションが無効になります。",
        ...
        heatmapSize: {
            ...
            ja: "目的のヒートマップのセルスケールに合わせるため、config.style.layout.width と config.style.layout.height の新しい属性を調整してください。",
...
<template>
    <div
        class="w-full rounded-xl my-6 relative overflow-x-auto dark:bg-[#42d39210] bg-gradient-to-br from-transparent to-[#42d39240] shadow-[inset_0_2px_2px_#FFFFFF,0_4px_6px_rgba(0,0,0,0.1)] dark:shadow-[inset_0_2px_2px_#4A4A4A,0_4px_6px_rgba(0,0,0,0.5)] px-4"
    >
        <VueDataUi
        ...
        >
            <template #title="{ color }">
                <div class="flex flex-row place-items-center gap-2">
                    <VueUiIcon
                        ...
                        :stroke="!isDarkMode ? '#1d915d' : '#42d392'"
                    />
            ...
            <template #content>
                <div class="w-full p-4 text-black dark:text-white">
                    <ul>
                        <li
                            class="border-l-2 border-app-green pl-4 mb-4"
                            v-if="padding"
                        >
                            {{ translations.padding[store.lang] }}
                        </li>
                        <li
                            class="border-l-2 border-app-green pl-4 mb-4"
                            v-if="debug"
                        >
                            {{ translations.debug[store.lang] }}
                        </li>
                        <li
                            class="border-l-2 border-app-green pl-4 mb-4"
                            v-if="cssAnimation"
                        >
                            {{ translations.cssAnimation[store.lang] }}
                        </li>
                        <li
                            class="border-l-2 border-app-green pl-4 mb-4"
                            v-if="zoomPreview"
                        >
                            {{ translations.zoomPreview[store.lang] }}
                        </li>
                        <li
                            class="border-l-2 border-app-green pl-4 mb-4"
                            v-if="autoRotate"
                        >
                            {{ translations.autoRotate[store.lang] }}
                        </li>
                        <li
                            class="border-l-2 border-app-green pl-4 mb-4"
                            v-if="xyTransitions"
                        >
                            {{ translations.xyTransitions[store.lang] }}
                        </li>
                        <li
                            class="border-l-2 border-app-green pl-4 mb-4"
                            v-if="heatmapSize"
                        >
                            {{ translations.heatmapSize[store.lang] }}
                        </li>
                        <slot />
                    </ul>
                </div>
