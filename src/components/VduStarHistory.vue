...
const sourceWithLastStar = computed(() => {
    const sortedSource = [...source.value].sort((a, b) => {
        return (
            dateToTimestamp(a.snapshot_date) - dateToTimestamp(b.snapshot_date)
        );
    });
...
const normalizedSource = computed(() => {
    return sourceWithLastStar.value.flatMap((current, index) => {
        ...
        const daysBetween = Math.round(
            (nextTimestamp - currentTimestamp) / DAY_MS,
        );
        ...
        return Array.from({ length: daysBetween }, (_, dayIndex) => {
            ...
            return {
                snapshot_date: timestampToDate(
                    currentTimestamp + dayIndex * DAY_MS,
                ),
    ...
    return [
        {
            ...
            shape: "star",
            smooth: true,
            temperatureColors: isDarkMode.value
                ? ["#fdd663", "#42d392"]
                : ["#42d392", "#5f8aee"],
...
<template>
    <div
        :class="`p-4 bg-[#FFFFFF] dark:bg-[#1A1A1A] rounded-md mb-4 relative ${isDarkMode ? 'dark-mode' : 'light-mode'}`"
    >
        ...
        >
                    ...
                    <polygon
                        :points="
                            createStar({
                                plot: {
                                    x:
                                        svg.drawingArea.left +
                                        svg.drawingArea.width / 2,
                                    y:
                                        svg.drawingArea.top +
                                        svg.drawingArea.height / 2,
                                },
                                radius: svg.drawingArea.height / 3,
                            })
                        "
                        fill="#66666610"
                    />
            ...
            <template #area-gradient="{ series, id: gradientId }">
                <linearGradient :id="gradientId" x1="0" x2="0" y1="0" y2="1">
                    ...
                    <stop
                        offset="100%"
                        :stop-color="isDarkMode ? '#1A1A1A' : '#FFFFFF'"
                        stop-opacity="0"
                    />
                </linearGradient>
            </template>
