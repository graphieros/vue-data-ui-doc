...
const url =
    "https://api.github.com/repos/graphieros/vue-data-ui/issues?state=all";
const issues = ref([]);
...
const closedIssuesWordCloud = computed(() => {
    ...
    return createWordCloudDatasetFromPlainText(
        removeNonAlphabetic(clean(titles)),
...
<template>
    ...
    <BaseCard>
        <div
            class="mt-4 w-full flex flex-col gap-2 max-h-[800px] overflow-y-auto p-2"
        >
            <h2 class="text-xl">Open issues</h2>
            <div
                v-if="issues.length === 0"
                class="flex flex-col gap-2 mb-5 place-items-center justify-center w-full"
            >
                <VueUiIcon
                    name="wrench"
                    :stroke="isDarkMode ? '#5A5A5A' : '#8A8A8A'"
                    :size="36"
                />
                <span class="text-[#8A8A8A] dark:text-[#7A7A7A]"
                    >No issues for now</span
                >
            </div>
            <div
            ...
            >
                <div
                    class="flex flex-row flex-wrap gap-2 pb-2 border-b border-gray-500 dark:border-[#3A3A3A] px-2"
                >
                ...
                <div
                    class="flex flex-row flex-wrap gap-4 pt-2 border-t border-gray-500 dark:border-[#3A3A3A] px-2"
                >
                    <template v-for="reaction in reactions">
                        <div
                            v-if="issue.reactions[reaction.description]"
                        ...
                        >
                            ...
                            <span class="text-xs">{{
                                issue.reactions[reaction.description]
                            }}</span>
                        </div>
                    </template>
                </div>
