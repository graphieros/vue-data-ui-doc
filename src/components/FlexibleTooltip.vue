...
<template>
    <div :class="`group cursor-pointer relative inline-block overflow-visible`">
        ...
        <div
            :class="
                tpClass +
                ' shadow-[inset_0_2px_2px_#FFFFFF,0_4px_6px_rgba(0,0,0,0.1)] dark:shadow-[inset_0_2px_2px_#4A4A4A,0_4px_6px_rgba(0,0,0,0.5)] rounded-lg'
            "
            v-if="!mute"
        >
            <div class="flex flex-col gap-2">
                ...
                >
                    <slot name="before" />
                    <div
                        class="flex flex-col gap-2 place-items-center w-full justify-center"
                    >
            ...
            <svg
                v-if="position === 'top'"
                :class="`absolute text-gray-100 ${props.alwaysBlue ? 'dark:text-app-blue' : 'dark:text-[#3A3A3A]'} h-2 w-full left-0 top-full`"
                x="0px"
                y="0px"
                viewBox="0 0 255 255"
                xml:space="preserve"
            ...
            <svg
                ...
                :class="`absolute text-gray-100 ${props.alwaysBlue ? 'dark:text-app-blue' : 'dark:text-[#3A3A3A]'} h-2 w-full left-0 bottom-full`"
                x="0px"
                y="0px"
                viewBox="0 0 255 255"
                xml:space="preserve"
            >
                <polygon
                    class="fill-current"
                    points="127.5,127.5 255,255, 0,255"
                />
            </svg>
            <svg
                v-if="position === 'right'"
                :class="`absolute text-gray-100 ${props.alwaysBlue ? 'dark:text-app-blue' : 'dark:text-[#3A3A3A]'} h-2 top-1/2 -translate-y-1/2 -left-2 bottom-full`"
                x="0px"
                y="0px"
                viewBox="0 0 10 10"
                xml:space="preserve"
            ...
            <svg
                v-if="position === 'left'"
                :class="`absolute text-gray-100 ${props.alwaysBlue ? 'dark:text-app-blue' : 'dark:text-[#3A3A3A]'} h-2 top-1/2 -translate-y-1/2 -right-2 bottom-full`"
                x="0px"
                y="0px"
                viewBox="0 0 10 10"
                xml:space="preserve"
