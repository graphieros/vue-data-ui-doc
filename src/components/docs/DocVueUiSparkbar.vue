...
<template>
    <div>
        <h1 class="text-center font-satoshi-bold text-app-blue mb-12 text-2xl">VueUiSparkbar</h1>
        <div :class="`transition-all mx-auto ${isFixed ? 'fixed bottom-0 w-[300px] left-0 z-50 overflow-auto border border-black dark:border-white bg-gray-100 dark:bg-[rgb(26,26,26)] shadow-xl' : 'sm:w-1/2'}`">
            ...
            <div class="w-full flex place-items-center place-content-center mb-6 mt-12">
                <button class="flex gap-1 bg-gradient-to-br from-app-green to-app-blue py-3 px-5 rounded-md text-white hover:shadow-xl dark:text-black font-satoshi-bold hover:from-app-blue hover:to-app-green transition-all" @click="copyToClipboard(mainConfig.vue_ui_sparkbar)"><CopyIcon/> Copy default config as JSON</button>
            </div>
        </div>
        <Box>
            <template #tab0>
    ...
    [
        {
            name: string;
            value: number;
            prefix?: string; <span class="text-gray-500">// to show a symbol before the value label</span>
            suffix?: string; <span class="text-gray-500">// to show a symbol after the value label</span>
            rounding?: number;
        }
    ]
</code>
