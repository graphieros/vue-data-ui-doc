...
const colorWheelContent = computed(
    () => `import { createColorWheel } from "vue-data-ui/utils;
...
const createTSpanTemplate = computed(
    () => `<text
    :x="10"
    :y="20"
    :font-size="16"
...
const bindingsTemplate = computed(
    () => `<template>
    <div>
        <label>
            Background color:
    ...
    return [
        {
            ...
            series: applyDataCorrection(
                ds.map((d) => ({ value: d })),
                {
                    averageWindow: _averageWindow.value,
                    smoothingTau: _smoothingTau.value,
                },
            ).map((d) => d.value),
        },
    ];
...
<template>
    <div
        :class="`hidden lg:block fixed top-[82px] ${isMenuOpen ? 'left-[300px]' : 'left-[60px]'} z-10 p-4 bg-[#FAFAFA] dark:bg-[#3A3A3A] shadow-md`"
        :style="{
    ...
    >
        <div class="flex flex-row flex-wrap gap-2 leading-[1px] pr-12 text-xs">
            <div
                :class="`hover:shadow-md font-mono p-2 rounded-full transition-all ${link === activeSectionId ? 'bg-app-blue text-[#1A1A1A] shadow-md' : 'bg-gray-200 dark:bg-[#4A4A4A] hover:bg-gray-100 dark:hover:bg-[#5A5A5A]'}`"
                v-for="link in utilityMenu"
            >
                <a class="cursor-pointer" @click="scrollToSection(link)">{{
                    link
                }}</a>
            </div>
        </div>
    ...
    <div>
        ...
        >
            ...
            <span
                ><span class="text-black dark:text-app-blue-light">{{
                    translations.utilityFunctions[store.lang]
                }}</span></span
            >
        </h1>

        <BaseDocDescription
            :text="translations.utilityFunctionsDescription[store.lang]"
        />
        ...
        <BaseCard class="font-mono mt-12">
            ...
            <p
                class="mt-2 px-8 text-gray-500 dark:text-app-blue-light font-inter"
            >
                All utility functions can be directly imported from
                "vue-data-ui"
            </p>
            <p
                class="mt-2 px-8 text-gray-500 dark:text-app-blue-light font-inter"
            >
                Since v3.15.9, utility functions and their types are treeshaken,
        ...
        <BaseCard class="mt-6" id="applyDataCorrection">
            <div class="p-4" dir="auto">
                ...
                <p class="mt-2 text-gray-500 dark:text-app-blue-light">
                    {{ utilityTranslations.applyDataCorrection[store.lang] }}
                </p>

                <div class="p-4 overflow-auto">
                    <div
                        class="flex flex-col gap-4 flex-wrap max-w-[600px] mx-auto mb-4"
                    >
                        <label class="flex flex-col">
                            <code>averageWindow ({{ _averageWindow }})</code>
                            <code
                                class="text-xs mb-2 text-gray-500 dark:text-[#8A8A8A]"
                                >{{ comments.averageWindow[store.lang] }}</code
                            >
                            <input
                                v-model="_averageWindow"
                                type="range"
                                :min="0"
                                :max="20"
                        ...
                        <label class="flex flex-col">
                            ...
                            <code
                                class="text-xs mb-2 text-gray-500 dark:text-[#8A8A8A]"
                                >{{ comments.smoothingTau[store.lang] }}</code
                            >
                            <input
                                v-model="_smoothingTau"
                                type="range"
                                :min="0"
                                :max="20"
                                class="accent-app-blue"
                    ...
                    <div
                        class="mx-auto max-w-[600px] p-4 bg-white dark:bg-[#1A1A1A] rounded mb-4"
                    >
                        <VueUiXy
        ...
        <BaseCard class="mt-6" id="useObjectBindings">
            <div class="p-4" dir="auto">
                ...
                <p class="mt-2 text-gray-500 dark:text-app-blue-light">
                    {{ utilityTranslations.useObjectBindings[store.lang] }}
                </p>
            </div>
            ...
            <div class="mx-auto max-w-[300px] my-4">
                ...
                <VueUiDonut :dataset="donutSet" :config="donutConfig">
                </VueUiDonut>
                <div class="mt-2">
                    <a
        ...
        <BaseCard class="mt-6">
            <div class="p-4" dir="auto" id="mergeConfigs">
                ...
                <p class="mt-2 text-gray-500 dark:text-app-blue-light">
                    {{ utilityTranslations.mergeConfigs[store.lang] }}
                </p>
            </div>

            <div class="p-4 overflow-auto">
                <CodeParser
                    :content="mergeConfigsContent"
                    language="javascript"
                    @copy="store.copy()"
                />
            </div>
            ...
            <div class="p-4" dir="auto" id="getVueDataUiConfig">
                ...
                <p class="mt-2 text-gray-500 dark:text-app-blue-light">
                    {{ utilityTranslations.mergeConfigsExample[store.lang] }}
                </p>
            </div>

            <div class="p-4 overflow-auto">
                <CodeParser
                    :content="getVueDataUiConfigAndMergeConfigsContent"
                    language="javascript"
                    @copy="store.copy()"
                />
            </div>
        ...
        <BaseCard class="mt-6" id="abbreviate">
            <div class="p-4" dir="auto">
                ...
                <p class="mt-2 text-gray-500 dark:text-app-blue-light">
                    {{ utilityTranslations.abbreviate[store.lang] }}
                </p>
            </div>
            ...
            <div class="p-4 flex flex-col gap-2 w-full">
                ...
                <div class="flex flex-col gap-1 w-fit">
                    <label class="text-xs" for="abbrLen"
                        >Abbrevation length:</label
                    >
                    <BaseNumberInput
                        v-model:value="abbrLen"
                        :min="1"
                        :max="12"
                        labelId="abbrLen"
                    />
        ...
        <BaseCard class="mt-6" id="darkenColor">
            <div class="p-4" dir="auto">
                ...
                <p class="mt-2 text-gray-500 dark:text-app-blue-light">
                    {{ utilityTranslations.darkenColor[store.lang] }}
                </p>
            </div>

            <div class="p-4 overflow-auto">
                <CodeParser
                    :content="darkenContent"
                    language="javascript"
                    @copy="store.copy()"
                />
            </div>
            ...
            <div class="p-4 flex flex-col gap-2 w-fit">
                ...
                >
                    <label
                        for="range-dark"
                        class="text-xs z-0 pointer-events-none bg-[#4A4A4A] dark:bg-black px-2 rounded-lg min-w-[64px] text-center text-white tabular-nums"
                        >Darken strength</label
                    >
                    <input
                        id="range-dark"
                        type="range"
                        v-model="strengthDark"
                        :min="0"
                        :max="1"
                        :step="0.01"
                        class="accent-app-blue z-0"
        ...
        <BaseCard class="mt-6" id="lightenColor">
            <div class="p-4" dir="auto">
                ...
                <p class="mt-2 text-gray-500 dark:text-app-blue-light">
                    {{ utilityTranslations.lightenColor[store.lang] }}
                </p>
            </div>

            <div class="p-4 overflow-auto">
                <CodeParser
                    :content="lightenContent"
                    language="javascript"
                    @copy="store.copy()"
                />
            </div>
            ...
            <div class="p-4 flex flex-col gap-2 w-fit">
                ...
                >
                    <label
                        for="range-light"
                        class="text-xs z-0 pointer-events-none bg-[#4A4A4A] dark:bg-black px-2 rounded-lg min-w-[64px] text-center text-white tabular-nums"
                        >Lighten strength</label
                    >
                    <input
                        id="range-light"
                        type="range"
                        v-model="strengthLight"
                        :min="0"
                        :max="1"
                        :step="0.01"
                        class="accent-app-blue z-0"
        ...
        <BaseCard class="mt-6" id="shiftColorHue">
            <div class="p-4" dir="auto">
                ...
                <p class="mt-2 text-gray-500 dark:text-app-blue-light">
                    {{ utilityTranslations.shiftColorHue[store.lang] }}
                </p>
            </div>

            <div class="p-4 overflow-auto">
                <CodeParser
                    :content="shiftColorContent"
                    language="javascript"
                    @copy="store.copy()"
                />
            </div>
        ...
        <BaseCard class="mt-6" id="createTSpans">
            <div class="p-4" dir="auto">
                ...
                <p class="mt-2 text-gray-500 dark:text-app-blue-light">
                    {{ utilityTranslations.createTSpans[store.lang] }}
                </p>
            </div>

            <div class="p-4 overflow-auto">
                <CodeParser
                    :content="createTSpanContent"
                    language="javascript"
                    @copy="store.copy()"
                />
                <CodeParser
                    :content="createTSpanTemplate"
                    language="html"
                    @copy="store.copy()"
                />
            </div>
        ...
        <BaseCard class="mt-6" id="getCumulativeAverage">
            <div class="p-4" dir="auto">
                ...
                <p class="mt-2 text-gray-500 dark:text-app-blue-light">
                    {{ utilityTranslations.getCumulativeAverage[store.lang] }}
                </p>
            </div>
            <div class="p-4 overflow-auto">
                <CodeParser
                    :content="getCumulativeAverageTemplate"
                    language="javascript"
                    @copy="store.copy()"
                />
            </div>
        ...
        <BaseCard class="mt-6" id="getCumulativeMedian">
            <div class="p-4" dir="auto">
                ...
                <p class="mt-2 text-gray-500 dark:text-app-blue-light">
                    {{ utilityTranslations.getCumulativeMedian[store.lang] }}
                </p>
            </div>
            <div class="p-4 overflow-auto">
                <CodeParser
                    :content="getCumulativeMedianTemplate"
                    language="javascript"
                    @copy="store.copy()"
                />
            </div>
