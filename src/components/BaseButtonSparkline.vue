    ...
    <div class="flex flex-col">
        ...
        >
            <template #button-content>
                <div
                    class="w-[200px] h-[40px] py-2 tabular-nums flex flex-row gap-2 justify-between px-3 place-items-center"
                >
                    <label class="text-xs absolute top-0.5 left-3">{{
                        title
                    }}</label>
                    <span class="text-sm mt-3"
                        >{{ sparklineDataset[selectedIndex].period }}:</span
                    >
                    <div class="flex flex-row gap-0 place-items-center">
                        <span class="text-2xl">{{
                            sparklineDataset[selectedIndex].value
                        }}</span>
                        <slot name="icon" />
                    </div>
                </div>
