...
<template>
    <div class="relative ra-special overflow-visible">
                ...
                <template #title="{ isOpen }">
                    ...
                    >
                        <div class="flex flex-row place-items-center gap-2">
                            <VueUiIcon
                                :class="`${isOpen ? 'rotate-90' : ''} transition-transform ml-2`"
                                name="arrowRight"
                                :size="12"
                                :stroke="
                                    isDarkMode
                        ...
                        <div
                            :class="`shadow-[inset_0_2px_2px_#FFFFFF,0_4px_6px_rgba(0,0,0,0.1)]
        dark:shadow-[inset_0_2px_2px_#4A4A4A,0_4px_6px_rgba(0,0,0,0.5)] absolute top-1/2 right-3 -translate-y-1/2 text-xs rounded-full bg-[#FFFFFF50] dark:bg-[#FFFFFF10] text-black dark:text-app-blue font-inter-medium w-[24px] h-[24px] flex place-items-center justify-center`"
                        >
                            {{ items.length }}
                        </div>
                    </div>
        ...
        <div
            v-show="!_isOpen"
            class="absolute highlight pointer-events-none opacity-0 transition-opacity duration-200 peer-hover:opacity-100"
        />
        <div
            v-show="!_isOpen"
            class="absolute highlight-shade pointer-events-none opacity-0 transition-opacity duration-200 peer-hover:opacity-100"
        />
        <div
            v-show="!_isOpen"
            class="absolute highlight-spark1 pointer-events-none opacity-0 transition-opacity duration-200 peer-hover:opacity-100"
        />
    </div>
</template>
