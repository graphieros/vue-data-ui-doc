...
function formatSuggestion(word) {
    ...
    return word.replace(
        regex,
        '<span class="font-inter-medium bg-app-blue-light dark:bg-app-blue text-black">$1</span>',
    );
}
...
<template>
    ...
    >
        <form autocomplete="off" onsubmit="return false;">
            <div class="flex flex-row gap-2">
                <div class="anim-wrapper w-[280px] rounded-full -mt-[1px]">
                    <input
                        ...
                        v-model="searchTerm"
                        :placeholder="
                            store.translations.search.placeholder[store.lang]
                        "
                        list="attrSearch"
                    />
                </div>
                ...
                >
                    <VueUiIcon
                        name="magnify"
                        :stroke="isDarkMode ? '#87e6bb' : '#1A1A1A'"
                        :size="20"
                    />
                </button>
                <button
            ...
            <div
                ...
                class="font-inter-medium text-[14px] absolute top-0 -mt-[330px] left-7 rounded-t-lg tabular-nums bg-white dark:bg-[#3A3A3A] text-black dark:text-[#CCCCCC] w-fit px-2 py-1"
            >
    ...
    >
                ...
                <div class="flex flex-row gap-2">
                    ...
                    >
                        <option value="" disabled selected>
                            {{
                                store.translations.search.componentSelect[
                                    store.lang
                                ]
                            }}
                        </option>
                        <option v-for="el in objNames" class="text-left">
                            {{ el }}
                        </option>
                    </select>
                    <button
                        v-if="
                            (searchTerm || showSuggestions) && selectedObjName
                        "
                        class="h-[36px] w-[36px] flex place-items-center justify-center border border-gray-600 rounded-lg hover:bg-gradient-to-br hover:from-app-orange hover:to-orange-700 hover:border-app-orange text-black dark:text-app-orange dark:hover:text-white transition-colors hover:text-white disabled:opacity-50 disabled:cursor-not-allowed"
                        @click="
                            selectedObjName = '';
                    ...
                    >
                        ...
                        <template #content>
                            <div class="flex flex-col w-full">
                                ...
                                {{
                                    store.translations.search.defaultValue[
                                        store.lang
                                    ]
                                }}
                                :
                      ...
                      <div v-if="res.type === 'string' && res.value.includes('#')" :style="`background:${res.value}`" class="h-6 w-6 rounded border border-gray-400"/>
                                ...
                                >
                                    <div
                                        class="hover:underline dark:text-app-green font-black mt-2"
                                    >
                                        {{
                                            store.translations.search
                                                .viewComponent[store.lang]
                                        }}
                                    </div>
                                </router-link>
                            </div>
        ...
        <div
            v-if="!hasResults"
            class="mt-6 flex flex-row place-items-center gap-2"
        >
            <InfoCircleIcon />
            <span>
        ...
        <div
            class="w-full flex place-items-center justify-end absolute bottom-6 right-6"
        >
            <button
