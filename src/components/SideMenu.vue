...
function showTooltip(source) {
    ...
    if (source.route === "/docs#utility-functions") {
        ...
        tooltipContent.value = `<div class="flex flex-center flex-col gap-2 place-items-center"><div class="flex flex-row place-items-center"><span>VueUi</span><b class="text-app-blue">${source.tooltipContent}</b></div>${
            source.thumb
                ? `<img class="h-12 max-w-[150px]" src="${
                      isDarkMode.value ? source.thumb : source.thumbLight
                  }">`
                : ""
        }</div>`;
    }
...
/**
 * Autocomplete search over all sideMenuItems
 */
const searchInputQuery = ref("");
const isSearchMenuOpen = ref(false);
const activeSearchResultIndex = ref(0);

const flatSideMenuItems = computed(() => {
    return sideMenuItems.value.flatMap((group) =>
...
const filteredSideMenuItems = computed(() => {
    const query = searchInputQuery.value.trim().toLowerCase();
    if (!query) {
        return [];
    }

    return flatSideMenuItems.value
        .filter((item) => {
            return (
...
watch(searchInputQuery, (value) => {
    if (!value.trim()) {
        activeSearchResultIndex.value = 0;
        return;
    }
    if (activeSearchResultIndex.value >= filteredSideMenuItems.value.length) {
        activeSearchResultIndex.value = 0;
    }
});
...
function handleSearchBlur() {
    // Delay closing to allow click on an item
    setTimeout(() => {
...
function moveSearchSelection(step) {
    const length = filteredSideMenuItems.value.length;
    if (!length) {
        return;
    }
    const nextIndex = activeSearchResultIndex.value + step;
    if (nextIndex < 0) {
        activeSearchResultIndex.value = length - 1;
...
<template>
    ...
    <aside
        :class="`transition-all fixed left-0 top-[49px] ${
            isOpen ? 'w-[300px]' : 'w-[59px]'
        } h-[calc(100svh_-_114px)] bg-gray-300 dark:bg-black overflow-y-auto overflow-x-hidden border-r dark:border-[#2A2A2A] p-6 shadow-2xl z-20`"
    >
        ...
        dark:shadow-[inset_0_2px_2px_#4A4A4A,0_4px_6px_rgba(0,0,0,0.5)] ${
            router.currentRoute.value.fullPath === '/universal-component'
                ...
                <div class="relative mb-2">
                    <VueUiIcon name="zoomPlus" class="absolute top-2.5 left-3" :stroke="isDarkMode ? '#83a4f2' : '#8A8A8A'" />
                    <input
                        ref="componentSearch"
                    ...
                    <ul
                        ...
                        class="absolute left-0 right-0 mt-1 max-h-72 overflow-y-auto rounded-2xl border border-gray-300 dark:border-[#3A3A3A] bg-white dark:bg-[#151515] shadow-xl z-40"
                    >
                        <li
                            v-for="(item, index) in filteredSideMenuItems"
                            :key="item.route"
                            class="cursor-pointer px-3 py-2 text-sm flex flex-row items-center justify-between gap-3"
                            :class="
