...
<template>
    <div :class="`flex flex-row gap-4 w-full my-4 ${justify} flex-wrap`">
        <div class="relative" v-if="defaultConfig">
            <FlexibleTooltip
                position="bottom"
                :content="translations.docs.copyDefaultConfig[store.lang]"
        ...
        <div class="relative" v-if="customConfig">
            ...
            >
                <button
                    @click="copyToClipboard(customConfig)"
                    class="shadow-[inset_0_2px_2px_#FFFFFF,0_4px_6px_rgba(0,0,0,0.1)] dark:shadow-[inset_0_2px_2px_#4A4A4A,0_4px_6px_rgba(0,0,0,0.5)] h-[36px] w-[36px] sm:h-[50px] sm:w-[50px] flex place-items-center justify-center rounded-lg bg-gray-100 dark:bg-[#242424] hover:bg-[#FAFAFA] dark:hover:bg-[#2A2A2A] transition-colors"
                >
                    <VueUiIcon
                        name="settings"
        ...
        <div class="relative" v-if="customDataset">
            <FlexibleTooltip
                position="bottom"
                :content="translations.docs.copyThisDataset[store.lang]"
                width="w-fit min-w-[120px]"
                delay="delay-150"
            >
                <button
                    @click="copyToClipboard(customDataset)"
                    class="shadow-[inset_0_2px_2px_#FFFFFF,0_4px_6px_rgba(0,0,0,0.1)] dark:shadow-[inset_0_2px_2px_#4A4A4A,0_4px_6px_rgba(0,0,0,0.5)] h-[36px] w-[36px] sm:h-[50px] sm:w-[50px] flex place-items-center justify-center rounded-lg bg-gray-100 dark:bg-[#242424] hover:bg-[#FAFAFA] dark:hover:bg-[#2A2A2A] transition-colors"
                >
                    <VueUiIcon
                        name="numbers"
