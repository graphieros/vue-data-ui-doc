...
<template>
    <router-link
        :to="link"
        :class="{
            'link-disabled': isSelected(link) && !clickableWhenActive,
            'text-black-100 dark:text-gray-200': !isSelected(link),
            'text-black dark:text-app-green': isSelected(link),
        }"
    >
        <button
            @click="emit('click')"
        ...
        >
            <div
                v-if="isSelected(link)"
                class="bg-app-green h-3 w-3 rounded-full shadow-[inset_0_2px_2px_#FFFFFF,0_4px_6px_rgba(0,0,0,0.1)] dark:shadow-[inset_0_2px_2px_#CCCCCC,0_4px_6px_rgba(0,0,0,0.5)]"
            />
