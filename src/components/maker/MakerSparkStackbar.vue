...
const CONFIG_MODEL = ref(
    JSON.parse(JSON.stringify(defaultData.vue_ui_sparkstackbar.model)),
);
...
onMounted(() => {
    ...
    if (localStorage.sparkStackbarDataset) {
        ...
        localStorage.setItem(
            "sparkStackbarDataset",
            JSON.stringify(defaultData.vue_ui_sparkstackbar.dataset),
        );
    }
...
function resetModel() {
    CONFIG_MODEL.value = JSON.parse(
        JSON.stringify(defaultData.vue_ui_sparkstackbar.model),
    );
    step.value += 1;
...
<template>
    <div>
        ...
        <div class="overflow-x-auto text-xs max-w-[800px] mx-auto">
            <ComponentContent
                :dataset="
                    datasetItems.map(({ name, value, color }) => {
