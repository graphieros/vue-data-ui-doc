...
const CONFIG_MODEL = ref(
    JSON.parse(JSON.stringify(defaultData.vue_ui_history_plot.model)),
);
...
onMounted(() => {
    ...
    if (localStorage.historyPlotDataset) {
        ...
        localStorage.setItem(
            "historyPlotDataset",
            JSON.stringify(defaultData.vue_ui_history_plot.dataset),
        );
    }
    step.value += 1;
});
...
function resetModel() {
    CONFIG_MODEL.value = JSON.parse(
        JSON.stringify(defaultData.vue_ui_history_plot.model),
    );
    step.value += 1;
...
function addDatapoint(seriesIndex) {
    datasetItems.value[seriesIndex].values.push(
        JSON.parse(JSON.stringify(options.value.datasetItems.values[0])),
    );
    saveDatasetToLocalStorage();
}

function deleteDatapoint(seriesIndex, datapointIndex) {
    datasetItems.value[seriesIndex].values = datasetItems.value[
        seriesIndex
    ].values.filter((dp, i) => i !== datapointIndex);
    saveDatasetToLocalStorage();
}
...
<template>
    <div>
            ...
            <details open>
                ...
                <div class="flex flex-col gap-2">
                    <div
                        ...
                        :class="`w-fit bg-gray-200 dark:bg-[#FFFFFF10] overflow-x-auto overflow-y-visible relative shadow dark:shadow-md p-3 pl-6 rounded flex flex-row gap-3`"
                        ...
                        <table>
                            ...
                            <tbody>
                                <tr>
                                    ...
                                    <td>
                                        <div
                                            class="flex flex-row gap-2 overflow-scroll max-w-[800px]"
                                        >
                                            <div
                                                v-for="(dp, j) in ds.values"
                                                class="relative"
                                            >
                                                <div
        ...
        <div class="overflow-x-auto text-xs max-w-[800px] mx-auto">
            <ComponentContent
                :dataset="
                    datasetItems.map(({ name, values, color }) => {
