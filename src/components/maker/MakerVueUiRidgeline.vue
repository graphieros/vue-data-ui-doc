...
const CONFIG_MODEL = ref(
    JSON.parse(JSON.stringify(defaultData.vue_ui_ridgeline.model)),
);
...
onMounted(() => {
    ...
    if (localStorage.ridgelineDataset) {
        ...
        localStorage.setItem(
            "ridgelineDataset",
            JSON.stringify(defaultData.vue_ui_ridgeline.dataset),
        );
    }
    step.value += 1;
});
...
const maxSeries = computed(() => {
    return Math.max(
        ...datasetItems.value.flatMap((ds) =>
            ds.datapoints.flatMap((dp) => dp.values.length),
        ),
    );
});
...
function resetModel() {
    CONFIG_MODEL.value = JSON.parse(
        JSON.stringify(defaultData.vue_ui_ridgeline.model),
    );
    step.value += 1;
...
function addDatapoint(datasetIndex) {
    ...
    datasetItems.value[datasetIndex].datapoints.push({
        ...JSON.parse(JSON.stringify(options.value.datapoint)),
    });
    if (wasSingle) {
        datasetItems.value[datasetIndex].datapoints[
            datasetItems.value[datasetIndex].datapoints.length - 1
        ].name = datasetItems.value[0].datapoints[0].name;
    }
    saveDatasetToLocalStorage();
}
...
function deleteDatapoint(datasetIndex, datapointIndex) {
    datasetItems.value[datasetIndex].datapoints = datasetItems.value[
        datasetIndex
    ].datapoints.filter((el, i) => i !== datapointIndex);
    saveDatasetToLocalStorage();
}

function deleteDatapointValue(datasetIndex, datapointIndex, valueIndex) {
    datasetItems.value[datasetIndex].datapoints[datapointIndex].values =
        datasetItems.value[datasetIndex].datapoints[
            datapointIndex
        ].values.filter((el, i) => i !== valueIndex);
    saveDatasetToLocalStorage();
}
                        ...
                        <table>
                            <thead>
                                <tr>
                                    ...
                                    <th
                                        class="text-left text-xs flex flex-row place-items-center gap-4"
                                    >
                                        {{
                                            makerTranslations.datapoints[
                                                store.lang
                                            ]
                                        }}
                                        <BaseButton
                            ...
                            <tbody>
                                <tr>
                                    ...
                                    <td>
                                        <div
                                            class="border border-gray-500 p-2 rounded bg-[#1A1A1A10] dark:bg-[#FFFFFF10]"
                                        >
                                            <table>
                                                <thead>
                                                    <tr>
                                                            ...
                                                            >
                                                                <table>
                                                                    <thead>
                                                                        <tr>
                                                                            <th
                                                                                class="text-left text-xs flex flex-row place-items-center gap-2"
                                                                            >
                                                                                {{
                                                                                    makerTranslations
                                                                                        .values[
                                                                                                ...
                                                                                                v-if="
                                                                                                    !CONFIG_MODEL.find(
                                                                                                        (
                                                                                                            ...
                                                                                                            el.key ===
                                                                                                            'style.chart.xAxis.labels.values',
                                                                                                    )
                                                                                                        .def[
                                                                                                        k
                                                                                                    ]
