...
const CONFIG_MODEL = ref(
    JSON.parse(JSON.stringify(defaultData.vue_ui_spark_trend.model)),
);
...
onMounted(() => {
    ...
    if (localStorage.sparkTrendDataset) {
        ...
        localStorage.setItem(
            "sparkTrendDataset",
            JSON.stringify(defaultData.vue_ui_spark_trend.dataset),
        );
    }
    step.value += 1;
});
...
function resetModel() {
    CONFIG_MODEL.value = JSON.parse(
        JSON.stringify(defaultData.vue_ui_spark_trend.model),
    );
    step.value += 1;
