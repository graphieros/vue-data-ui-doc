...
const CONFIG_MODEL = ref(
    JSON.parse(JSON.stringify(defaultData.vue_ui_circle_pack.model)),
);
...
onMounted(() => {
    ...
    if (localStorage.circlePackDataset) {
        ...
        localStorage.setItem(
            "circlePackDataset",
            JSON.stringify(defaultData.vue_ui_circle_pack.dataset),
        );
    }
    step.value += 1;
});
...
function resetModel() {
    CONFIG_MODEL.value = JSON.parse(
        JSON.stringify(defaultData.vue_ui_circle_pack.model),
    );
    step.value += 1;
