...
const CONFIG_MODEL = ref(
    JSON.parse(JSON.stringify(defaultData.vue_ui_chestnut.model)),
);
...
onMounted(() => {
    ...
    if (localStorage.chestnutDataset) {
        ...
        localStorage.setItem(
            "chestnutDataset",
            JSON.stringify(defaultData.vue_ui_chestnut.dataset),
        );
    }
...
function resetModel() {
    CONFIG_MODEL.value = JSON.parse(
        JSON.stringify(defaultData.vue_ui_chestnut.model),
    );
    step.value += 1;
...
function addDatasetItem() {
    datasetItems.value.push({
        ...JSON.parse(JSON.stringify(options.value.root)),
        id: createUid(),
        branches: [JSON.parse(JSON.stringify(options.value.branch))],
    });
    step.value += 1;
...
function pushToRoot({ rootId }) {
    ...
    thisItem.branches.push({
        ...JSON.parse(JSON.stringify(options.value.branch)),
        id: createUid(),
        breakdown: [JSON.parse(JSON.stringify(options.value.breakdown))],
    });
    step.value += 1;
    saveDatasetToLocalStorage();
}
...
function pushToBranch({ rootId, branchId }) {
    ...
    thisBranch.breakdown.push(
        JSON.parse(JSON.stringify(options.value.breakdown)),
    );
    step.value += 1;
    saveDatasetToLocalStorage();
}
...
function updateBranchValue({ branchId, rootId }) {
    const thisRoot = datasetItems.value.find((_) => _.id === rootId);
    const thisBranch = thisRoot.branches.find((_) => _.id === branchId);
    thisBranch.value = thisBranch.breakdown
        .map((b) => b.value)
        .reduce((a, b) => a + b, 0);
    saveDatasetToLocalStorage();
}
            ...
            <details open>
                ...
                <div class="flex flex-col gap-2">
                    <div
                        ...
                        :class="`w-full overflow-x-auto overflow-y-visible relative shadow dark:shadow-md p-3 pl-6 rounded flex flex-row gap-3 place-items-center`"
                        ...
                        <table>
                            <thead>
                                <tr>
                                    ...
                                    <th class="text-left text-xs">
                                        {{
                                            makerTranslations.labels.branch[
                                                store.lang
                                            ]
                                        }}
                                    </th>
                            ...
                            <tbody>
                                <tr>
                                    ...
                                    <td v-for="(branch, j) in ds.branches">
                                        <div
                                            class="p-2 pt-8 rounded bg-[#FFFFFF33] dark:bg-[#FFFFFF10] mx-2 shadow-md relative flex flex-row place-items-center"
                                        >
                                            <BaseButton
                                            ...
                                            <table>
                                                ...
                                                <tbody>
                                                    <tr>
                                                        ...
                                                        >
                                                            <div
                                                                class="flex flex-col gap-4 p-2 bg-[#FFFFFF33] dark:bg-[#FFFFFF10] rounded mx-1 shadow-md relative"
                                                            >
                                                                <BaseButton
                                                                    color="error"
                                                                    :size="4"
