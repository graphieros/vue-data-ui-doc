...
const CONFIG_MODEL = ref(
    JSON.parse(JSON.stringify(defaultData.vue_ui_galaxy.model)),
);
...
onMounted(() => {
    ...
    if (localStorage.galaxyDataset) {
        ...
        localStorage.setItem(
            "galaxyDataset",
            JSON.stringify(defaultData.vue_ui_galaxy.dataset),
        );
    }
    step.value += 1;
});
...
function resetModel() {
    CONFIG_MODEL.value = JSON.parse(
        JSON.stringify(defaultData.vue_ui_galaxy.model),
    );
    step.value += 1;
