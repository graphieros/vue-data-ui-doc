...
const CONFIG_MODEL = ref(
    JSON.parse(JSON.stringify(defaultData.vue_ui_world.model)),
);
...
function getCountryCodesFromGeoJSON(geojson, codeKey = "ISO_A3") {
    ...
    const keyToUse =
        features.length > 0
            ? possibleKeys.find((k) => k in (features[0].properties || {})) ||
              codeKey
            : codeKey;
...
onMounted(() => {
    ...
    if (localStorage.worldDataset) {
        ...
        localStorage.setItem(
            "worldDataset",
            JSON.stringify(defaultData.vue_ui_world.dataset),
        );
    }
    step.value += 1;
});
...
function resetModel() {
    CONFIG_MODEL.value = JSON.parse(
        JSON.stringify(defaultData.vue_ui_world.model),
    );
    step.value += 1;
