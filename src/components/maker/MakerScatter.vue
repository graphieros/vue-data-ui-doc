...
const CONFIG_MODEL = ref(
    JSON.parse(JSON.stringify(defaultData.vue_ui_scatter.model)),
);
...
onMounted(() => {
    ...
    if (localStorage.scatterDataset) {
        ...
        localStorage.setItem(
            "scatterDataset",
            JSON.stringify(defaultData.vue_ui_scatter.dataset),
        );
    }
...
function resetModel() {
    CONFIG_MODEL.value = JSON.parse(
        JSON.stringify(defaultData.vue_ui_scatter.model),
    );
    step.value += 1;
...
function addDatasetItem() {
    datasetItems.value.push({
        ...JSON.parse(JSON.stringify(options.value.datasetItem)),
        id: createUid(),
        values: [
            {
                ...JSON.parse(JSON.stringify(options.value.valueItem)),
                id: createUid(),
            },
        ],
    });
...
<template>
    <div>
        ...
        <details open>
            ...
            <div class="flex flex-col gap-2">
                <div
                    ...
                    :class="`w-full overflow-x-auto overflow-y-visible relative shadow dark:shadow-md p-3 pl-6 rounded flex flex-row place-items-center gap-3`"
                ...
                >
                            ...
                            <tr>
                                ...
                                <td>
                                    <select
                                        ...
                                        @change="saveDatasetToLocalStorage"
                                    >
                                        <option v-for="opt in shapeOptions">
                                            {{ opt }}
                                        </option>
                                    </select>
                                </td>
                                <td v-for="dp in ds.values">
                                    <div class="flex flex-col gap-2">
                                        ...
                                        <div class="flex flex-col gap-2 mt-2">
                                            <label class="text-left text-xs">{{
                                                makerTranslations.labels.weight[
                                                    store.lang
                                                ]
                                            }}</label>
                                            <input
                    ...
                    <BaseButton
                        ...
                        :tooltip="
                            translations.maker.tooltips.addData[store.lang]
                        "
                    >
                        <PlusIcon />
                    </BaseButton>
        ...
        <div class="overflow-x-auto text-xs max-w-[800px] mx-auto">
            <ComponentContent
                :dataset="
                    datasetItems.map(({ name, values, color, shape }) => {
                        return {
                            name,
                            values: values.map((v) => {
                                return {
