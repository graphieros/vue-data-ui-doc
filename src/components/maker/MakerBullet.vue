...
const CONFIG_MODEL = ref(
    JSON.parse(JSON.stringify(defaultData.vue_ui_bullet.model)),
);
...
onMounted(() => {
    ...
    if (localStorage.bulletDataset) {
        ...
        localStorage.setItem(
            "bulletDataset",
            JSON.stringify(defaultData.vue_ui_bullet.dataset),
        );
    }
    step.value += 1;
});
...
function resetModel() {
    CONFIG_MODEL.value = JSON.parse(
        JSON.stringify(defaultData.vue_ui_bullet.model),
    );
    step.value += 1;
...
function deleteSegment(index) {
    dataset.value.segments = dataset.value.segments.filter(
        (s, i) => i !== index,
    );
    saveDatasetToLocalStorage();
}
            ...
            <details open>
                ...
                <div class="flex flex-col gap-2">
                    <div class="flex flex-row gap-2 place-items-center">
                        <button
                            class="py-2 px-5 rounded border border-black dark:border-app-blue hover:bg-[#5f8aee20] transition-colors"
                            @click="randomizeValue"
                        >
                            Random value
                        </button>
                        : {{ dataset.value.toFixed(2) }}
                    </div>
                    <div class="flex flex-row gap-2 place-items-center">
                        <label for="target">Target:</label>
                        <input
                            type="number"
                            v-model="dataset.target"
                            @change="saveDatasetToLocalStorage"
                        />
                    </div>
                    <div
                        v-for="(ds, i) in dataset.segments"
                                ...
                                <tr>
                                    <td>
                                        <input
                                            class="h-[36px]"
                                            type="text"
                                            v-model="dataset.segments[i].name"
                                            @change="saveDatasetToLocalStorage"
                                        />
                                    </td>
                                    <td>
                                        <input
                                            class="h-[36px]"
                                            type="number"
                                            v-model="dataset.segments[i].from"
                                            @change="saveDatasetToLocalStorage"
                                        />
                                    </td>
                                    <td>
                                    ...
                                    <td>
                                        <input
                                            type="color"
                                            v-model="dataset.segments[i].color"
                                            @change="saveDatasetToLocalStorage"
                                        />
                                    </td>
                                </tr>
