...
const CONFIG_MODEL = ref(
    JSON.parse(JSON.stringify(defaultData.vue_ui_gauge.model)),
);
...
onMounted(() => {
    ...
    if (localStorage.gaugeDataset) {
        ...
        localStorage.setItem(
            "gaugeDataset",
            JSON.stringify(defaultData.vue_ui_gauge.dataset),
        );
    }
    step.value += 1;
});
...
function resetModel() {
    CONFIG_MODEL.value = JSON.parse(
        JSON.stringify(defaultData.vue_ui_gauge.model),
    );
    step.value += 1;
