...
const CONFIG_MODEL = ref(
    JSON.parse(JSON.stringify(defaultData.vue_ui_flow.model)),
);
...
onMounted(() => {
    ...
    if (localStorage.flowDataset) {
        ...
        localStorage.setItem(
            "flowDataset",
            JSON.stringify(defaultData.vue_ui_flow.dataset),
        );
    }
    step.value += 1;
});
...
function resetModel() {
    CONFIG_MODEL.value = JSON.parse(
        JSON.stringify(defaultData.vue_ui_flow.model),
    );
    step.value += 1;
