...
const CONFIG_MODEL = ref(
    JSON.parse(JSON.stringify(defaultData.vue_ui_funnel.model)),
);
...
onMounted(() => {
    ...
    if (localStorage.funnelDataset) {
        ...
        localStorage.setItem(
            "funnelDataset",
            JSON.stringify(defaultData.vue_ui_funnel.dataset),
        );
    }
    step.value += 1;
});
...
function resetModel() {
    CONFIG_MODEL.value = JSON.parse(
        JSON.stringify(defaultData.vue_ui_funnel.model),
    );
    step.value += 1;
