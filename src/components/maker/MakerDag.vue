...
const CONFIG_MODEL = ref(
    JSON.parse(JSON.stringify(defaultData.vue_ui_dag.model)),
);
...
onMounted(() => {
    ...
    if (localStorage.dagDataset) {
        ...
        localStorage.setItem(
            "dagDataset",
            JSON.stringify(defaultData.vue_ui_dag.dataset),
        );
    }
    step.value += 1;
});
...
function resetModel() {
    CONFIG_MODEL.value = JSON.parse(
        JSON.stringify(defaultData.vue_ui_dag.model),
    );
    step.value += 1;
...
function deleteNode(nodeId) {
    datasetItems.value.nodes = datasetItems.value.nodes.filter(
        (n) => n.id !== nodeId,
    );
    datasetItems.value.edges = datasetItems.value.edges.filter(
        (n) => n.from !== nodeId && n.to !== nodeId,
    );
    saveDatasetToLocalStorage();
}
...
function deleteEdge(edge) {
    datasetItems.value.edges = datasetItems.value.edges.filter(
        (e) => `${e.from}${e.to}` !== `${edge.from}${edge.to}`,
    );
    saveDatasetToLocalStorage();
}
</script>
                    ...
                    <div v-for="node in datasetItems.nodes">
                        <div class="flex flex-row gap-2 place-items-center">
                            <div class="min-w-[64px]">
                                ...
                                <code class="text-[10px]">{{ node.id }}</code>
                            </div>
                            <div>
                                <label>
                    ...
                    <div v-for="edge in datasetItems.edges">
                        <div class="flex flex-row gap-2">
                            <label>
                                ...
                                (id:
                                <code class="text-[10px]">{{ edge.from }}</code
                                >)
                            </label>
                            →
                            <label>
                                ...
                                (id:
                                <code class="text-[10px]">{{ edge.to }}</code
                                >)
                            </label>
                            <label>
                                <span class="font-inter-medium mr-2"
                                    >animated</span
                                >
                                <input
                                    type="checkbox"
