...
const CONFIG_MODEL = ref(
    JSON.parse(JSON.stringify(defaultData.vue_ui_stackbar.model)),
);
...
onMounted(() => {
    ...
    if (localStorage.bigStackbarDataset) {
        ...
        localStorage.setItem(
            "bigStackbarDataset",
            JSON.stringify(defaultData.vue_ui_stackbar.dataset),
        );
    }
    step.value += 1;
});
...
function resetModel() {
    CONFIG_MODEL.value = JSON.parse(
        JSON.stringify(defaultData.vue_ui_stackbar.model),
    );
    step.value += 1;
