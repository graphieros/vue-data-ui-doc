...
const CONFIG_MODEL = ref(
    JSON.parse(JSON.stringify(defaultData.vue_ui_word_cloud.model)),
);
...
onMounted(() => {
    ...
    if (localStorage.wordCloudDataset) {
        ...
        localStorage.setItem(
            "wordCloudDataset",
            JSON.stringify(defaultData.vue_ui_word_cloud.dataset),
        );
    }
    step.value += 1;
});
...
function resetModel() {
    CONFIG_MODEL.value = JSON.parse(
        JSON.stringify(defaultData.vue_ui_word_cloud.model),
    );
    step.value += 1;
...
<template>
    <div>
        ...
        <BaseCard>
            <details open>
                ...
                <div class="flex flex-col gap-2">
                    <textarea
                        v-if="isDarkMode"
                        class="p-4 !bg-[#1A1A1A] h-[200px] rounded-md"
                        v-model="datasetItems"
                        @change="step += 1"
                    ></textarea>
                    <textarea
                        v-else
                        class="p-4 !bg-white h-[200px] rounded-md"
                        v-model="datasetItems"
                        @change="step += 1"
                    ></textarea>
                </div>
        ...
        <div class="overflow-x-auto text-xs max-w-[800px] mx-auto">
            <ComponentContent
                :dataset="
                    createWordCloudDatasetFromPlainText(datasetItems, (word) =>
                        word.toLowerCase(),
    ...
    <Transition name="fade">
        ...
        >
            <VueUiWordCloud
                :dataset="
                    createWordCloudDatasetFromPlainText(datasetItems, (w) =>
                        w.toLowerCase(),
                    )
                "
                :config="finalConfig"
