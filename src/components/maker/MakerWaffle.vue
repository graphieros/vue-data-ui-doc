...
const CONFIG_MODEL = ref(
    JSON.parse(JSON.stringify(defaultData.vue_ui_waffle.model)),
);
...
onMounted(() => {
    ...
    if (localStorage.waffleDataset) {
        ...
        localStorage.setItem(
            "waffleDataset",
            JSON.stringify(defaultData.vue_ui_waffle.dataset),
        );
    }
    step.value += 1;
});
...
function resetModel() {
    CONFIG_MODEL.value = JSON.parse(
        JSON.stringify(defaultData.vue_ui_waffle.model),
    );
    step.value += 1;
