...
const CONFIG_MODEL = ref(
    JSON.parse(JSON.stringify(defaultData.vue_ui_radar.model)),
);
...
onMounted(() => {
    ...
    if (localStorage.radarDataset) {
        ...
        localStorage.setItem(
            "radarDataset",
            JSON.stringify(defaultData.vue_ui_radar.dataset),
        );
    }
    if (localStorage.radarCategories) {
        ...
        localStorage.setItem(
            "radarCategories",
            JSON.stringify(defaultData.vue_ui_radar.categories),
        );
    }
    step.value += 1;
});
...
function resetModel() {
    CONFIG_MODEL.value = JSON.parse(
        JSON.stringify(defaultData.vue_ui_radar.model),
    );
    step.value += 1;
...
function addDatasetItem() {
    datasetItems.value.push({
        ...
        id: createUid(),
        values: categoryItems.value.map((_) => 0),
    });
    step.value += 1;
    saveDatasetToLocalStorage();
}
...
function addCategoryItem() {
    categoryItems.value.push({
        ...JSON.parse(JSON.stringify(options.value.categoryItems)),
        id: createUid(),
    });
...
<template>
    <div>
        ...
        <BaseCard>
            <details open>
                <summary class="cursor-pointer mb-4">
                    {{ makerTranslations.categoriesLabel[store.lang] }}
                </summary>
                <div class="flex flex-col gap-2">
                    <div
                        v-for="(ds, i) in categoryItems"
                ...
                <div class="flex flex-row gap-4 mt-4 mb-6">
                    <BaseButton
                        ...
                        :tooltip="
                            translations.maker.tooltips.addCategory[store.lang]
                        "
            ...
            <details open>
                ...
                <div class="flex flex-col gap-2">
                    <div
                        ...
                        :class="`w-full overflow-x-auto overflow-y-visible relative shadow dark:shadow-md p-3 pl-6 rounded flex flex-row gap-3 bg-gray-200 dark:bg-[#2A2A2A]`"
                    >
                        <BaseButton
                            color="error"
                            :size="6"
                        ...
                        <table>
                            <thead>
                                <tr>
                                    ...
                                    <th class="text-left text-xs">
                                        {{
                                            makerTranslations.labels.target[
                                                store.lang
                                            ]
                                        }}
                                    </th>
        ...
        <div class="overflow-x-auto text-xs max-w-[800px] mx-auto">
            <ComponentContent
                :dataset="{
                    categories: categoryItems.map(({ name, color }) => {
                        return { name, color };
                    }),
                    series: datasetItems.map(
                        ({ name, values, color, target }) => {
                            return { name, values, color, target };
                        },
                    ),
                }"
                :config="finalConfig"
