...
const CONFIG_MODEL = ref(
    JSON.parse(JSON.stringify(defaultData.vue_ui_chord.model)),
);
...
onMounted(() => {
    ...
    if (localStorage.chordDataset) {
        ...
        localStorage.setItem(
            "chordDataset",
            JSON.stringify(defaultData.vue_ui_chord.dataset),
        );
    }
    step.value += 1;
});
...
function resetModel() {
    CONFIG_MODEL.value = JSON.parse(
        JSON.stringify(defaultData.vue_ui_chord.model),
    );
    step.value += 1;
...
function addDatasetItem() {
    ...
    datasetItems.value.matrix.push(
        new Array(datasetItems.value.matrix.length + 1).fill(0),
    );
    datasetItems.value.labels.push("?");
    datasetItems.value.colors.push(
        JSON.parse(JSON.stringify(palette))[
            datasetItems.value.matrix.length + 1
        ],
    );
    saveDatasetToLocalStorage();
}
                        ...
                        <table>
                            <thead>
                                <tr>
                                    ...
                                    <th v-for="(ds, i) in datasetItems.matrix">
                                        <div
                                            class="relative pt-6 flex flex-col"
                                        >
                                            <BaseButton
                                                color="error"
                                                :size="4"
                                            ...
                                            <input
                                                type="color"
                                                v-model="datasetItems.colors[i]"
                                                class="w-[100px]"
                                            />
                                            <input
                                                type="text"
                                                v-model="datasetItems.labels[i]"
                                                class="w-[100px]"
                                            />
                                        </div>
                                    </th>
                            ...
                            <tbody>
                                <tr v-for="(ds, i) in datasetItems.matrix">
                                    <td>
                                        <div
                                            class="flex flex-row align-center place-items-center gap-2"
                                        >
                                            <BaseButton
                                                color="error"
                                                :size="4"
                                    ...
                                    >
                                        <input
                                            type="number"
                                            :min="0"
                                            v-model="datasetItems.matrix[i][j]"
                                            class="w-[100px]"
                                        />
                                    </td>
                                </tr>
