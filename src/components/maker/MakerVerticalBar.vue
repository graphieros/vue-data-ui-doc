...
const CONFIG_MODEL = ref(
    JSON.parse(JSON.stringify(defaultData.vue_ui_horizontal_bar.model)),
);
...
onMounted(() => {
    ...
    if (localStorage.horizontalBarDataset) {
        ...
        localStorage.setItem(
            ...
            JSON.stringify(defaultData.vue_ui_horizontal_bar.dataset),
        );
    }
...
function resetModel() {
    CONFIG_MODEL.value = JSON.parse(
        JSON.stringify(defaultData.vue_ui_horizontal_bar.model),
    );
    step.value += 1;
...
function addDatasetItem() {
    datasetItems.value.push({
        ...JSON.parse(JSON.stringify(options.value.parent)),
        id: createUid(),
    });
    step.value += 1;
...
function updateParent({ parentId }) {
    const thisParent = datasetItems.value.find((_) => _.id === parentId);
    thisParent.value = thisParent.children
        .map((c) => c.value)
        .reduce((a, b) => a + b, 0);
    step.value += 1;
    saveDatasetToLocalStorage();
...
<template>
    <div>
                    ...
                    >
                        ...
                        <div class="flex flex-col gap-2 place-items-end">
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <div
                                                ...
                                                :key="`${ds.id}_${j}`"
                                                class="relative p-2 bg-[#FFFFFF33] dark:bg-[#FFFFFF10] rounded shadow-md mb-2"
                                            >
                                                <BaseButton
                                                    color="error"
                                                    :size="4"
                            ...
                            <BaseButton
                                color="success"
                                fab
                                @click="addChild({ parentId: ds.id })"
                                :tooltip="
                                    translations.maker.tooltips.addData[
                                        store.lang
                                    ]
                                "
                            >
        ...
        <div class="overflow-x-auto text-xs max-w-[800px] mx-auto">
            <ComponentContent
                :dataset="
                    datasetItems.map(({ name, value, color, children }) => {
