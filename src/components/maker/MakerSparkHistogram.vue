...
const CONFIG_MODEL = ref(
    JSON.parse(JSON.stringify(defaultData.vue_ui_sparkhistogram.model)),
);
...
onMounted(() => {
    ...
    if (localStorage.sparkHistogramDataset) {
        ...
        localStorage.setItem(
            "sparkHistogramDataset",
            JSON.stringify(defaultData.vue_ui_sparkhistogram.dataset),
        );
    }
...
function resetModel() {
    CONFIG_MODEL.value = JSON.parse(
        JSON.stringify(defaultData.vue_ui_sparkhistogram.model),
    );
    step.value += 1;
            ...
            <details open>
                ...
                <div class="flex flex-row gap-2 w-full overflow-auto">
                    <div
                        ...
                        :class="`shadow dark:shadow-md p-3 rounded flex flex-row gap-3 bg-gray-200 dark:bg-[#FFFFFF10]`"
                    ...
                    >
                        <div class="relative flex flex-col gap-2">
                            ...
                            <div class="flex flex-col gap-2">
                                <label class="text-sm text-left"
                                    >{{
                                        translations.maker.labels.period[
                                            store.lang
                                        ]
                                    }}
                                    :
                                    {{
                                        makerTranslations.labels.labels[
                                            store.lang
                                        ]
                                    }}</label
                                >
                            ...
                            <div class="flex flex-col gap-2">
                                <label class="text-sm text-left"
                                    >{{
                                        makerTranslations.labels.opacity[
                                            store.lang
                                        ]
                                    }}
                                    :
                                ...
                                <input
                                    class="h-[36px] w-[100px] accent-app-blue"
                                    type="range"
                                    min="0"
                                    max="1"
                                    step="0.01"
                                    v-model="ds.intensity"
                                    @change="saveDatasetToLocalStorage"
                                />
                            </div>
                        </div>
                ...
                :dataset="
                    datasetItems.map(
                        ({ value, valueLabel, timeLabel, intensity }) => {
                            return { value, valueLabel, timeLabel, intensity };
                        },
                    )
                "
                :config="finalConfig"
