...
watch(
    ...
    async (list) => {
        const next = list.map((m) => ({
            ...
            duration:
                typeof m.duration === "number" ? m.duration : DEFAULT_DURATION,
...
<template>
    <Teleport to="body">
        <TransitionGroup
            name="toast"
            tag="div"
            class="fixed bottom-0 left-0 z-[2147483647] p-6 flex flex-col gap-3 pointer-events-none"
            aria-live="polite"
        >
            <BaseCard
            ...
            >
                ...
                <!-- Progress bar -->
                <div
                    class="absolute left-0 bottom-0 h-[6px] w-full overflow-hidden rounded-b-xl"
                    :class="
                        message.type === 'error'
                            ? 'bg-red-500/10'
                            : 'bg-emerald-500/10'
                ...
                >
                    <div
                        ...
                        :class="[
                            ...
                            message.type === 'error'
                                ? 'bg-app-red'
                                : 'bg-gradient-to-r from-app-green to-app-green-dark',
                        ]"
                        :style="{
                            '--duration': (message.duration || 0) + 'ms',
                        }"
                        aria-hidden="true"
                    />
                </div>
