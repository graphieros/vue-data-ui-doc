...
<template>
    <div class="mx-auto max-w-[1250px] px-6 mt-6">
        <h2 class="text-center mx-auto max-w-[60ch] mb-6">
            {{ presentation[store.lang] }}
        </h2>
        <BaseCard class="sm:flex sm:flex-col gap-4">
            <div
                class="sm:flex sm:flex-row gap-4 place-content-center place-items-center justify-center mb-4 sm:mb-2"
            >
                <div
                    class="w-full sm:w-1/2 flex flex-col gap-4 place-items-center justify-center mb-4 sm:mb-0"
                >
                    <BaseColorInput
                        labelAsCode
                        v-model:value="knobs.colorBackground"
                        label="colorBackground"
                        labelId="colorBackground"
                    />
                    <BaseColorInput
                        labelAsCode
                        v-model:value="knobs.colorTextPrimary"
                        label="colorTextPrimary"
                        labelId="colorTextPrimary"
                    />
                    <BaseColorInput
                        labelAsCode
                        v-model:value="knobs.colorTextSecondary"
                        label="colorTextSecondary"
                        labelId="colorTextSecondary"
                    />
                    <BaseColorInput
                        labelAsCode
                        v-model:value="knobs.colorGrid"
                        label="colorGrid"
                        labelId="colorGrid"
                    />
                    <BaseColorInput
                        labelAsCode
                        v-model:value="knobs.colorBorder"
                        label="colorBorder"
                        labelId="colorBorder"
                    />
                ...
                <BaseCard
                    class="w-full h-fit sm:w-1/2 flex flex-row place-items-center justify-center"
                    :style="`backgroundColor: ${knobs.colorBackground} !important;`"
                >
                    <VueUiXy :dataset="dataset" :config="config" />
                </BaseCard>
            </div>
