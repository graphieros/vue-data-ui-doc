...
const colorBridgeDescription = ref({
    ...
    ja: "色は文化によって異なる意味を持つことがあります。文化的な文脈に基づいて地域固有のカラーパレットを生成するツールを作成しました。これにより、さらに適切なデータストーリーテリングが可能になります。",
...
const description = ref({
    ...
    ja: "config.customPalette（string[]）を使って独自のカラーパレットを設定できます。データセットで指定された色は、常にカスタムおよびデフォルトのパレットより優先されます。利用可能な色形式：HEX、RGB、HSL、色名。config.customPalette属性は以下のコンポーネントで使用できます：",
