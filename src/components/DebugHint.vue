...
const code = computed(() => `const config = computed<${props.configType}>(() => ({
...
<template>
    <div class="p-4 mb-6 rounded-xl bg-[#5f8aee50]">
      <div class="flex flex-row place-items-center gap-4">
        <VueUiIcon name="lightBulbOn" :stroke="isDarkMode ? '#FFFFFF' : '#1A1A1A'"/>
        <h3 class="font-inter-medium text-xl">{{ translations.h3[store.lang] }}</h3>
      </div>
