...
<template>
    <div class="my-4 grid grid-cols-1 md:grid-cols-2 gap-6 place-items-center w-full p-4 bg-gray-200 dark:bg-[#3A3A3A] rounded-md">
        ...
        <div class="w-full shadow-md">
            ...
            <VueDataUi component="VueUiScatter" :dataset="dataset" :config="{...config, theme: 'celebration'}" />
        </div>
        <div class="w-full shadow-md">
            <ThemeTag type="celebrationNight"/>
            <VueDataUi component="VueUiScatter" :dataset="dataset" :config="{...config, theme: 'celebrationNight'}" />
        </div>
    </div>
</template>