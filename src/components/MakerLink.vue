...
<template>
    <div class="relative" v-if="simple">
        <FlexibleTooltip
            position="bottom"
            :content="translations.menu.chartBuilder[store.lang]"
    ...
    <button
        v-else
        @click="returnToMaker"
        :class="`flex flex-row gap-2 place-items-center rounded-md border border-black dark:border-app-green ${props.size} hover:bg-[#42d39233] hover:shadow-xl`"
    >
        <ToolIcon />{{ translations.menu.chartBuilder[store.lang] }}
    </button>
</template>
